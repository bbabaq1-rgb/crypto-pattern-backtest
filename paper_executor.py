...
# 실거래 포지션 사이징 규칙
MAX_LIVE_POS   = 13    # 동시 최대 실거래 포지션. 5→12(2026-07-06) → 16(2026-09-05) → **13(2026-09-18 사용자 결정
                       # "리스크 1.5%로 올리고 맥스포스 13으로 같이 내려줘")**. RISK_FRAC 1.5% 상향과 한 쌍이다 —
                       # 건당 명목가가 equity 의 18.75% 라 총명목 상한(sizing.MAX_TOTAL_NOTIONAL_FRAC 2.5배)이
                       # **13.3개**에서 걸린다. 16 을 그대로 두면 14번째부터 슬롯이 아니라 상한 스킵으로 바뀔 뿐이다.
                       # 13슬롯 증거금 = equity x 81%(lev3). 16 시절 근거(격자 run 33961480251: 12→16 슬롯스킵
                       # 462→47, Calmar 1.84→1.85)는 risk 1.5% 에서는 총명목 상한이 먼저 걸려 성립하지 않는다.
                       # **주의: 이 값을 내려도 이미 열린 포지션은 청산하지 않는다** — 진입만 막힌다(아래 진입 루프).
                       # 적용 시점에 15건이 열려 있었으므로 13 밑으로 줄어들 때까지 신규 진입이 없다.
