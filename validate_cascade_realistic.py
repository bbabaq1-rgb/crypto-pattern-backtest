...
**꼬리가 두꺼운 분포**다. 2026-09-01 실측(daily_scheduler 스케줄 실행 100건):
    ...
    중앙값 16.0분 · p75 33.2분 · p90 172.9분 · 최대 231.5분 · 60분 이내 82%

절반은 16분 안에 도는데 18%가 1시간을 넘고 일부는 3~4시간까지 간다. 고정값으로는
이 혼합을 표현할 수 없다 — 평균만 보면 44.6분이지만 그 값을 전 신호에 씌우는 것은
실제 분포와 전혀 다르다.
...
def main():
    ...
    # 체결 관례를 먼저 밝힌다. 동결 프레임은 '주문 시각이 속한 봉의 종가'로 체결한다.
    # 1h 격자에서는 아무리 빨라도(1분) 다음 봉 종가 = d=1 이 되고, 실측 중앙값
    # 16분도 같은 d=1 이다. 즉 **1시간 크론과 상시 서버는 신호의 82%에서 같은 칸에
    # 떨어지고**, 차이는 1시간을 넘는 18% 꼬리뿐이다. d=0(=+2.43%)은 어떤 폴링
    # 인프라로도 도달 불가한 1차 검증의 기준선이며, 실제 체결은 d=0~d=1 사이다.
    print("  [체결 관례] 주문이 속한 1h 봉의 종가로 체결 — 최대 1시간 보수적.")
    print("  1h 격자는 지연 1분이어도 d=1. 실측 중앙 16분도 d=1 → 서버와 같은 칸.")
    print("  차이는 60분을 넘는 18% 꼬리(p90 173분)에서만 난다.\n")
