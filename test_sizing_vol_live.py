...
check("채택 위험 수준 고정: RISK_FRAC 1.5% / LEV_CAP 3 (사용자 결정 2026-09-18 재상향)",
...
# 고변동 신호가 최소증거금 미달로 잘리기 시작하는 equity 를 **파라미터에서 유도**해 고정한다.
# 하드코딩하면 RISK_FRAC 을 바꿀 때 조용히 무의미해진다(실제로 1%->1.5% 상향에서 그랬다).
FLOOR_SCALE = sz.VOL_LO / sz.VOL_S_NORM              # 최저 배율 = 최고변동 신호
eq_all_pass = sz.min_equity_for(FLOOR_SCALE, 0.08)   # 이 equity 이상이면 전 신호 통과
check("최저 배율에서도 통과하는 equity 문턱이 파라미터와 일치",
      abs(eq_all_pass - sz.MIN_MARGIN * sz.liq_safe_leverage(0.08) * 0.08
          / (sz.RISK_FRAC * FLOOR_SCALE)) < 1e-6, f"{eq_all_pass:.0f}")
check("문턱 위에서는 최고변동 신호도 주문이 나간다",
...
#   risk 1%/lev2 $355 → risk 1.5%/lev2 $237 → **risk 1.5%/lev3 $355** → risk 1%/lev3 $533
check(f"현재 파라미터(risk {sz.RISK_FRAC:.1%} / lev {sz.LEV_CAP})의 전신호 통과 문턱은 ${eq_all_pass:.0f}",
      abs(eq_all_pass - sz.MIN_MARGIN * sz.LEV_CAP * 0.08
          / (sz.RISK_FRAC * sz.VOL_LO / sz.VOL_S_NORM)) < 1.0, f"{eq_all_pass:.1f}")
check("현행 문턱은 $355 (2026-09-18 재상향으로 $533 에서 내려왔다)",
      abs(eq_all_pass - 355.0) < 1.0, f"{eq_all_pass:.1f}")
check("현 계좌 규모($606, 2026-09-18)에서는 어떤 변동성이든 스킵되지 않는다",
      all(sz.risk_based_size(606, 1e9, 0.08, vol_scale=v) is not None
          for v in (FLOOR_SCALE, 1.0, sz.VOL_HI / sz.VOL_S_NORM)))
# 경계를 고정한다 — equity 가 $355 밑으로 내려가면 고변동 신호부터 잘린다(현 여유 약 $251).
...
# risk 1.5% 에서 총 명목가 상한(2.5x)은 13.3개에서 걸린다 — 그래서 MAX_POS 를 13 으로 맞췄다.
# **두 상수는 한 쌍**이고, 한쪽만 바꾸면 슬롯 숫자가 허상이 되거나 상한이 먼저 걸린다.
import paper_executor as _pe
_cap_n = sz.MAX_TOTAL_NOTIONAL_FRAC / (sz.RISK_FRAC / 0.08)
check("총 명목가 상한 2.5x 도달 지점이 13.3개 (risk 1.5%)", abs(_cap_n - 13.3333) < 1e-3, _cap_n)
check("MAX_LIVE_POS 가 그 경계 바로 아래 — 슬롯이 진짜 상한이다",
      _pe.MAX_LIVE_POS <= _cap_n < _pe.MAX_LIVE_POS + 1, (_pe.MAX_LIVE_POS, round(_cap_n, 2)))
