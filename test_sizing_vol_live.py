...
check("채택 위험 수준 고정: RISK_FRAC 1.0% / LEV_CAP 3 (사용자 결정 2026-09-10 하향)",
      sz.RISK_FRAC == 0.010 and sz.LEV_CAP == 3)
...
check("현재 파라미터(risk 1.0% / lev 3)의 전신호 통과 문턱은 $533",
      abs(eq_all_pass - 532.5) < 1.0, f"{eq_all_pass:.1f}")
check("현 계좌 규모($763, 2026-09-10 입금 후)에서는 어떤 변동성이든 스킵되지 않는다",
      all(sz.risk_based_size(763, 1e9, 0.08, vol_scale=v) is not None
          for v in (FLOOR_SCALE, 1.0, sz.VOL_HI / sz.VOL_S_NORM)))
# 경계를 고정한다 — equity 가 $533 밑으로 내려가면 고변동 신호부터 잘린다(여유 약 $230).
check("equity 가 문턱 아래로 내려가면 최고변동 신호부터 스킵된다",
      sz.risk_based_size(500, 1e9, 0.08, vol_scale=FLOOR_SCALE) is None
      and sz.risk_based_size(500, 1e9, 0.08, vol_scale=1.0) is not None)
# 이번 하향이 산 것 — 총 명목가 상한(2.5x)에 걸리는 포지션 수. 13.3개(1.5%)에서 20개로.
check("총 명목가 상한 2.5x 가 MAX_POS 16 뒤로 물러난다 (20개)",
      abs(sz.MAX_TOTAL_NOTIONAL_FRAC / (sz.RISK_FRAC / 0.08) - 20.0) < 1e-9)
