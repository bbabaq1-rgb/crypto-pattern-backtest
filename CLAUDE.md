...
- **레짐별 분리 게이트** (2026-09-04, 사용자 제안 "레짐 나눠서 테스트해야 맞다"): report_regime_split.md.
  ...
  · **라우팅 부호 불일치 2건**: bull_altseason fvg 롱(실측 −2.12%, 숏 +1.39%) / bear fvg 숏
    (실측 −0.72%, 롱 +0.70%). 2026-06-24 표가 median/boot_p/OOS 없이 만들어진 결과.
  · **fvg 는 전 레짐 사망이 아니다** — bull_btc 에서 n=1,837 +2.23% med **+9.89%**(top30 OOS4/4),
    boot_p 만 못 넘음(상승장 무작위 롱을 못 이김). 죽은 곳은 altseason 과 2026 bear.
