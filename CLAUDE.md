...
- **사용자 강제 실행 — tp1_engulfing_1h, $30 (2026-09-09, 사용자 지시 "그냥 30달러만 강제 진행해볼수 있어? 지금 btc 들어가있는거 청산하고")**:
  ...
  · **손절 리셋 제거 (2026-09-19, 사용자 지시 "리셋 빼고 손실도 그대로 이어받게 해줘")**: registry
    `live_cap.reset_on_loss` true → **false**. 이제 손절도 익절과 같이 곱셈으로 누적한다 —
    포트 = 70 × Π(1+ret×3), 예외 없음. 2026-09-09 의 '손절나면 리셋' 규칙 철회.
    · **소급 적용이다** — 포트는 매 실행 **거래 이력 전체**로 다시 계산한다. 그래서 2026-09-16
      AAVE 손절(−7.98%, 저장 ret −8.18%, ×3 = **−24.5%**)이 리셋 대신 곱해지고, 포트가
      **약 $77.00 → $63.67** 로 내려간다(이후 승 4건 누적배수 1.100 은 그대로 곱해진다).
      다음 tp1 진입 로그의 `cap_margin` 으로 확인할 것.
    · **성격이 바뀐다** — 종전은 '위로는 복리, 아래로는 리셋'(손실이 계좌에서 나가고 원금 재시작)
      이라 포트가 $70 아래로 안 내려갔다. 이제는 아래로도 복리라 **연패 시 계속 줄어들고**,
      $10(MIN_MARGIN) 밑이면 주문이 아예 안 나간다(충전은 사용자 결정). 손절 1건이 −24.5% 라
      **익절 11건이 있어야 손절 1건을 되돌린다**(1.024^11 ≈ 1.31 vs 0.755).
    · 되돌리기: registry `reset_on_loss` 를 true 로. 코드 경로는 그대로 남아 있고 테스트가 고정한다.
    · test_tp1_live — 실측 AAVE 재현(승4 + 손절 → 약 $57.9)과 리셋 설정 동작을 둘 다 고정.
