...
- **레짐별 분리 게이트** (2026-09-04, 사용자 제안 "레짐 나눠서 테스트해야 맞다"): report_regime_split.md.
  ...
  · **레버리지·위험 격자 — 레버리지 상향 권고 안 함 (2026-09-04, 사용자 질문 "좀 더 공격적으로")**:
    ...
    · **LEV_CAP 2 → 3 상향 (2026-09-04, 사용자 결정, risk 1.5% 와 한 쌍)**: 2026-09-02 의 '레버리지
      ...
      · **2026-09-18 RISK_FRAC 1.0% → 1.5% 재상향 + MAX_POS 16 → 13 (사용자 결정 "리스크 1.5%로 올리고
        맥스포스 13으로 같이 내려줘")** — registry `risk_level_2026_09_18`. 계기는 사용자 관찰 '평균 진입
        포지션이 $20 로 너무 적다'. **먼저 정정한 사실**: $20 은 증거금이고 명목가는 3배($76)이며 15포지션
        명목가 합계가 약 $950 = equity 의 1.6배로 총 노출은 작지 않다. 그럼에도 건당을 키우는 레버는
        **RISK_FRAC 하나뿐**이다 — MAX_POS 는 사이징이 equity 기준이라 건당 크기와 무관하고, LEV_CAP 은
        증거금만 줄이고 명목가는 그대로다.
        · **두 상수는 한 쌍이다** — risk 1.5% 에서 건당 명목가가 equity 의 18.75% 라 총명목 상한(2.5배)이
          **13.3개**에서 걸린다. MAX_POS 16 을 그대로 두면 14번째부터 슬롯이 아니라 상한 스킵으로 바뀌어
          **슬롯 숫자가 허상**이 된다(9/05~9/18 의 16 이 그 상태였고 9/10 하향의 근거이기도 했다).
        · 되사는 것은 9/10 에 판 것 그대로: boot CAGR +49.1%→**+68.7%** / MDD중앙 −45.4%→**−60.8%** /
          p10 −65.1%→**−80.5%** / P(ruin) 0.3%→2.3% / Calmar 1.06→1.13. **5번 중 1번 −80% 낙폭**을
          받아들인 선택이고 **사전 기준(MDD중앙≥−35%)은 여전히 미충족**(통과 셀은 0.5% 뿐, 문턱 $1,065 라
          현 계좌에서 주문이 안 나간다). 게이트 문턱은 건드리지 않았다.
        · **부수: 최소주문 문턱이 $533 → $355** (배율 1.0 기준 $240 → $160). equity $606 기준 여유가
          $73 → $251 로 늘어 **2026-09-16 에 처음 넘어섰던 문턱 문제가 해소**된다.
        · **열린 포지션은 청산하지 않는다** — MAX_POS 하향은 진입만 막는다. 적용 시점 실포지션 15건이라
          13 밑으로 줄어들 때까지 신규 진입이 없다(청산은 방식D 규칙대로).
        · 테스트 단언을 **파라미터에서 유도**하도록 교체했다 — 하드코딩이 조용히 무의미해진 2026-09-04
          전례의 반복 방지. GRID_SLOTS 에 13 추가(현행이 격자 비교 기준이어야 한다).
      · 현행 요약: **RISK_FRAC 1.5% / LEV_CAP 3 / MAX_POS 13 / 손절 8% / 변동성 타겟팅 on**.
        명목가 = equity x 18.75% x vol_scale(0.45~1.80), 증거금 = 명목가/3 = equity x 6.25%.
...
- **BTC 엘리엇 임펄스 일일 브리핑 (2026-09-23, 사용자 지시 "매일 아침 9시에 진행 현황과 관점 브리핑")**:
  `elliott_count.json`(동결 카운트) + `elliott_watch.py`(일일 판정) + 09:00 KST(00:00 UTC) 트리거.
  **관찰·브리핑 전용 — 매매 경로와 완전히 단절**(test_elliott_watch §1 이 양방향 import 0·주문 함수 부재·
  registry/universe 미등재를 고정). 레포의 엘리엇 환원 패턴은 전부 게이트에서 기각됐고 이 건도 신호가 아니다.
  · **왜 동결하나** — 엘리엇의 고질병은 가격이 움직일 때마다 라벨을 조용히 바꿔 '항상 맞는' 카운트가 되는 것이다.
    라벨을 파일에 고정하면 재라벨이 커밋으로 남는다. `elliott_watch.py` 는 라벨을 못 바꾸고 판정만 한다
    (테스트가 '코드에 파동 가격·레벨 하드코딩 없음'을 고정 — 파일이 유일한 원천).
  · 동결 카운트(2026-09-23, 코인베이스 BTC-USD): 0파 57,718(07-01) → 1파 66,924(07-21) → 2파 62,210(08-01) →
    3파 82,283(09-03) → 4파 74,888(09-15) → 5파 **87,397**(09-21, 완성 미확정). 3대 규칙 전부 통과
    (4파-1파 중첩 여유 +7,964). 선행 구간은 2025-10-06 ATH 126,193 → 57,718 의 **조정형**(W-X-Y) — 5파 하락으로
    세면 4파 82,828 이 1파 저점 80,647 을 침범해 중첩 위반이라 임펄스 불가.
  · 시나리오 3개를 함께 들고 간다: **S1** 새 임펄스 1파(2파 0.382~0.786 = 76,059~64,069 뒤 3파 117k~128k) /
    **S2** 아직 더 큰 3파의 (v)(눌림 77,776~81,455) / **S3** 조정 미완의 A파(57,718 이탈 시 확정).
    **S3 는 반증 불가라 항상 살아 있다** — '카운트가 맞았다'를 사후에 주장하지 않기 위해 명시한다.
  · 데이터는 **코인베이스 BTC-USD 공개 캔들**(OKX 는 이 컨테이너에서 차단). 저점 날짜가 대표님 차트와 다를 수 있다
    — 57.7k 프린트는 코인베이스 07-01 57,717.55 / OKX 07-01 57,813.4 이고 6/26 저가는 58,243/58,333 이다.
  · 되돌리기: 트리거 삭제 한 번. 파일 3개는 매매와 무관해 남겨도 아무 동작을 안 한다.
  · **2파 A-B-C 추적 추가 (2026-09-25, 사용자 승인)**: `elliott_count.json["correction"]` — A 파 확정
    (87,397→82,709, **3파**: 하락 5파 읽기가 9/25 11:00Z 85,250 에서 4파 중첩으로 무효, ETH 도 같은 봉에서
    2,742.8 > 2,715 로 동시 무효) → 플랫 가설. 브리핑이 **B 가 A 의 몇 %**(최소 61.8% 85,606 / 90% 86,928)와
    C 목표(1.0~1.618A = 80.9k~82.7k)를 찍는다. **단일 플랫은 1파의 16~22% 에서 끝난다** — 38.2%(76,059)는
    C=2.42A, 61.8%(69,055)는 C=3.91A 라 단일 ABC 로 불가, W-X-Y 일 때만. 시간 비율(10/22~11/10)은 W-X-Y 지지.
    87,397 레벨은 '5파 연장 / 확장 플랫 B' 두 읽기 병기(ETH 4파 08-27→09-11→09-15 가 실제 확장 플랫). 라벨 무변경,
    revisions 에 사유 기록. ETH 교차 확인: 2파 8/01·4파 9/15·5파 9/21 **같은 날**, 5파 내부 6h 타임스탬프 동일.
...
- **실거래 안전장치** (2026-07-06): MAX_LIVE_POS **13**(2026-09-18 사용자 결정, RISK_FRAC 1.5% 와 한 쌍 —
  건당 명목가 18.75% 라 총명목 상한이 13.3개에서 걸린다. 종전 16 은 risk 1% 시절 값) ·
  종전 기록(사용자 승인 5→12 2026-07-06 → 16 2026-09-05, 슬롯 격자 근거 —
...
- **첫 실거래 손절 2건 — 변동성 타겟팅 사이징 실증 (2026-09-13~14)**: registry `stop_sizing_evidence_2026_09_13`.
  · **ETHFI** vol_awakening_4h 롱 9/13 22:03Z fill 0.6336 **−8.72%** 실현 **−$2.83** (shadow-R: stop −8.20% 2봉).
  · **RAY** fvg 롱 9/14 00:00Z fill 1.4229 **−7.69%** 실현 **−$2.40** (stop 1봉 — 진입 다음 봉).
  · **RAY 가 정확히 그 사례다** — 9/12 16:00Z `σ=178%/yr → 배율 하한 0.45 → 증거금 $10.68`(최소주문 $10 을
    $0.68 차 통과)로 들어간 건이 하루 만에 −8% 를 맞았는데 **위험 예산 `risk=$2.56` 대비 실현 −$2.40**.
    고변동 종목의 명목가를 깎아 손실을 예산 안에 묶는다는 2026-09-04 채택 근거가 실거래에서 처음 확인됐다.
    ETHFI 도 −$2.83 으로 같은 크기 — **σ 가 3배 달라도 건당 달러 손실은 붙어 있다.**
  · 둘 다 `reconcile-close` 로 정상 감지. 라벨 '손절(OKX algo)' 가 이 둘에서는 **맞다**(아래 라벨 결함은 수익 청산에만).
- **저가 코인 주문 실패 — 지수 표기, 수정 완료 (2026-09-18, 사용자 지시 "shib 같은 저가 코인 구매 실패 수정해줘")**:
  registry `low_price_coin_order_fail_2026_09_18`. **가격이 $0.0001 밑인 종목은 손절 등록이 구조적으로 거부돼
  진입이 항상 되돌려졌다** — SHIB/PEPE/BONK 급 전부. 9/17 SHIB 실측: 사이징까지 정상 통과한 뒤
  `sl_failed(okx {"code":"51000","msg":"Parameter slTriggerPx error"})_entry_reversed_ok`.
  · **원인은 파이썬 float 의 지수 표기** — `place_stop_algo` 가 `str(sl_px)` 로 파라미터를 만드는데
    파이썬은 1e-4 미만을 `str(9e-06) == '9e-06'` 으로 준다. OKX 가 그 문자열을 거부한다. SHIB 손절가
    9.384e-06 이 정확히 그 구간. **값은 옳았고 직렬화만 틀렸다** — 그래서 로그가 전부 정상으로 보였다.
    ccxt `price_to_precision` 도 tickSz 가 작으면 지수 표기를 돌려줄 수 있어 그 경로도 안전하지 않다.
  · **수정 3종**: ① `exchange.num_str` — 십진 고정 표기. slTriggerPx/tpTriggerPx/sz 전부 이걸로 만들어
    **진입 OCO·손절 누락 재등록·체결가 재정렬 세 경로를 한 곳에서** 덮는다. ② `place_swap_entry` 가
    손절가를 **진입 전에** 검증(`stop_px_invalid`/`stop_px_wrong_side`) — 종전은 진입 뒤 algo 거부에서야
    되돌려 신호마다 왕복 수수료를 버렸다. ③ `scheduler._px` — 신호 가격의 `round(x, 4)` 대체.
    종전에는 SHIB 신호가 **`@ 0.0 손절 0.0`** 으로 signals 행·알림·대시보드에 기록됐다(실주문은
    paper_executor 가 봉 종가에서 재계산하므로 체결에는 무관 — 표기·기록만의 결함).
  · **불변**: 신호 집합·사이징·슬롯·중복 방어·라우팅·배포 집합·청산 규칙. 정상 가격대는 문자열도
    주문도 종전과 동일(`num_str(460.0) == "460"`).
  · **이번 수정이 안 덮는 것**: 9/16 LINK `sl_failed(okx {"code":"1"...})` 는 **다른 원인**이다
    (LINK 는 지수 표기 구간이 아니다). code 1 은 일반 실패라 단서 부족 — 재발 시 전체 응답을 남겨 별도로 본다.
  · test_exchange_stops +5 / test_cron_split +5 / test_adopted_regimes 소스 고정 문자열 갱신.
- **겹친 종목 시장가 청산 — 메인 레그까지 닫혔다, 수정 완료 (2026-09-17, 사용자 지시 "전체 지금 수정")**:
  registry `shared_symbol_market_close_2026_09_17`. 2026-09-11 선택지 B 가 **OCO 경로만** 고쳤고 엔진
  시장가 청산 경로는 그대로 남아 있었다 — 9/14 ETH 때 "별개"라 적어둔 자리가 9/16 ONDO 에서 터졌다.
  · **(나) 확정 — 9/14 미해결 질문의 답**: 14:03Z 장부 11행 = 실포지션 11건(**정확히 일치, 유령 0**) →
    tp1 진입 12/12. 15:04Z `ONDO long D청산 실행 (atr_target)` 뒤 장부 10행 / 실포지션 **9건** —
    장부는 −2 인데 실포지션은 −3. 16:00Z reconcile 이 메인 행을 fill **0.33146**(= tp1 익절선
    0.3285×1.01) **−3.51% −$2.35** 로 기록, 점유 equal_lows_4h 3→2. 진입 시점에 장부·실포지션이
    정확히 일치했으므로 메인 레그는 **살아 있는 실포지션**이었다 → '유령 행 오귀속'(가) 배제.
  · **근본 원인은 `live_order` 유실** — push_positions_db 가 qty/leverage/sl_order_id 를 안 보내고
    restore_state_db 도 복원하지 않아, 러너에서 **진입 다음 실행부터** `pos["live_order"]` 가 None 이다.
    그래서 ① close_qty_for 가 `q=None` → **None(전량)** 을 반환해 병합된 net 포지션을 통째로 닫고
    ② sl_order_id 가 없어 'OCO 가 이미 집행됐는가' 가드까지 조용히 건너뛰었다. **두 결함이 같은 뿌리.**
  · **수정 3종**: ① `close_qty_for` → `(qty, frac)` 로 **전량 폴백 제거** — 단독 보유만 (None,None)=전량,
    나눠 들었으면 qty 를 알면 그 계약 수, 모르면 **명목가 지분 비율**, 지분조차 모르면 (None, 0.0) =
    **주문 보류**(다음 틱 재시도). ② `exchange.close_swap_position(..., qty_frac=)` 이 실포지션 수에
    비율을 곱하고 `amount_to_precision` 으로 반올림 — **계약 단위(ctVal)를 알 필요가 없고 레버리지
    추정 오차가 분자·분모에서 상쇄된다**(BTC 0.01 · ETH 0.1 같은 함정 회피). `qty_frac<=0` 은
    `share_unknown` 으로 주문을 아예 안 낸다. ③ `_live_order_from_db` + push 에 3컬럼 추가 —
    컬럼이 생기면 정확해지고 없어도 ②로 안전하게 돈다(schema_patch 에 추가, 사용자 실행).
  · **불변**: 진입·슬롯·중복 방어·사이징·라우팅·배포 집합·손절 주문 경로 전부. 단독 보유 행의 청산은
    종전과 동일(전량). test_tp1_live +7 / CI 전 목록 로컬 그린.
  · **잔여**: `ensure_stop_orders` 는 손절 **누락 시** 재등록에 아직 거래소 실포지션 전체를 쓴다
    (reduceOnly 라 위험은 없고 겹친 종목에서 algo 가 사라지는 드문 경우만) — 관찰 종료 후 별도 과제.
- **원장 결함 — 엔진 밖 전량 청산이 전부 '손절'로 기록된다 (2026-09-13 발견 → 2026-09-17 수정)**:
  registry `exit_label_defect_2026_09_13`. `paper_executor.py:863` 이 `reason = "손절(OKX algo)" if hist.get("type")
  in ("2","3","5")` 인데 OKX **type 2 는 '전량 청산'** 이지 손절이 아니다(3 청산, 5 ADL).
  · 오라벨 실증 2건: **BTC equal_lows_4h `손절(OKX algo) ·실거래 +0.56%`** · **ETH equal_lows_4h `손절 ·+2.27%`**
    (후자는 9/11 겹친 OCO 로 끊긴 그 메인 레그 — 가격수익 +2.48%, 수수료 차감 +2.27%). 정라벨 2건은 위 ETHFI·RAY.
  · **왜 중요한가**: 10/06 관찰 보고가 셀별 **손절 비율**을 집계한다(ih/marubozu/three_soldiers 정지·유지 근거).
    라벨이 틀리면 그 표가 틀린다.
  · **(a) 적용 완료 (2026-09-17)** — `exit_reason_of(pos, fill, okx_type)` 신설. type **3·4 → 강제청산(OKX)** ·
    **5 → ADL(OKX)** 로 따로 두고, 나머지(1·2)는 **체결가를 손절가·익절가와 대조**해 BARRIER_TOL(1.5%)
    안이면 그 배리어로, 아니면 **OKX청산**(수동·외부). 손익 부호가 어긋나면(수익인데 손절) 안 붙인다.
    · **문턱 1.5% 는 실측에서 나왔다** — 트리거 주문은 양쪽으로 빗나간다: ETHFI 는 손절선 **바깥**
      0.7%p(갭), RAY 는 **안쪽** 0.31%p. 한쪽만 잡는 규칙이면 RAY 를 'OKX청산' 으로 오판했을 것.
    · 실측 6건(오라벨 4 + 정라벨 2) + 익절·강제청산·ADL·숏·부호역행을 test_executor_safety 가 고정(11건).
    · **reason 문자열만 바뀐다 — 주문·청산·포지션 수명 무변경.**
  · **과거 원장의 오라벨 4건은 그대로 남는다** — 10/06 보고에서 손절 비율을 집계할 때 그 4건
    (BTC +0.56 / ETH +2.27 / SOL +0.74 / ETH −0.27)을 손절로 세지 말 것.
- **외부 트리거 첫 누락 — fast 09:03Z 1회 (2026-09-13)**: 2026-09-02 pg_cron 전환 이래 첫 미발화(08:03 → 10:03).
  영향은 그 1시간의 exit_spec 진입·청산 점검 누락뿐이고 **손절은 거래소 OCO 라 안전**. 9/13 13:03~9/15 00:03
  **36틱 연속 무결**로 단발 확인. 재발 시 `gh_dispatch_log`(401=PAT 만료) 확인 + 수동 dispatch 로 보충.
...
- **승률 35% 문턱 교정 감사 → INCONCLUSIVE, 그러나 증거는 완화 쪽 (2026-09-12, 사용자 지적 "건당·중앙·승률 다 보면
  너무 빡빡하다 / 최적화된 것도 아니다" + 지시 "돌려줘", run 34683465342)**: registry `winrate_guard_audit_prereg_2026_09_12` /
  report_winrate_audit.md. **문턱 35% 무변경, 실거래 무변경.**
  · **먼저 정정 — 중앙값은 이미 판정에서 빠져 있다**(2026-09-05 게이트 v2). 남은 실질 쟁점은 승률 하나였고 그것만 쟀다.
  · **판정 INCONCLUSIVE** — A2 적격 셀 11(하한 10)은 넘겼으나 **버킷당 3셀 하한**에서 걸림(<30% 0셀, 30-35% 2셀).
  · **A2(주 판정)가 단조 *감소*** — 승률 외 층을 전부 통과한 11셀에서 30-35% **+11.87%**(2) / 35-40% +7.63%(4) /
    **40%+ +1.98%**(5). 사전 등록한 INDEPENDENT 조건(승률↑→홀드아웃↑)의 **정반대**이고 문턱 바로 아래가 가장 좋다.
    A2b 진단(21셀)도 같은 모양 — 40%+ 가 +1.47% 로 최악.
  · **A3 거짓음성 2셀 실재** — triple_bottom_1d|bull_btc(승률 31%, 홀드아웃 **+20.29%** n=53) · triple_bottom_1d|bear
    (34%, +3.44% n=34). 승률만 아니었으면 전 층 통과. **유보 둘**: 둘 다 같은 패턴의 레짐 분해라 독립 증거는 사실상
    1건이고, **top5 기여도 41%** 로 복권형이 맞다 — 문턱이 걸러내려던 성질 자체는 실재한다.
  · **A4 격자** ≥25% 11셀 +5.84% / ≥30% 11셀 +5.84% / **≥35%(현행) 9셀 +4.50%** / ≥40% 5셀 +1.98% — 낮출수록 통과 셀도
    홀드아웃도 좋아진다(진단, 최대값 사후 선택 금지).
  · **A1 — 승률은 Calmar 가 아니라 복권형 지표와 겹친다**: rho(승률,Calmar) **−0.183 무관** / rho(승률,절사갭) **+0.786** /
    rho(승률,top5) **−0.573**. **내 사전 확률이 방향부터 오답** — 낙폭과 겹칠 거라 봤는데 실제로는 '소수 대박 의존'을
    직접 재는 지표와 겹친다. 그 지표들은 이미 게이트 안에 진단으로 있다.
  · **1차 실행(run 34682605766) A2·A3·A4 는 무효 — 내 결함, 인용 금지**: `frame_v3.perf_ok` 안에 `gate.dist_ok`(승률)가
    있는데 그걸 쓰는 `c2b_train` 을 '승률 외 층' 으로 세어 **저승률 셀이 C2b 도 자동 탈락** → A3 이 구조적으로 영원히
    빈 목록이 됐다('A3 없음' 은 데이터가 아니라 버그). **감사하려던 바로 그 오류를 감사 코드가 저질렀다** — 2026-09-09
    L8 B 벤치와 같은 구조. 수정 `c2b_no_winrate()`(train 게이트를 n·평균·boot_p 로만 재계산), **판정 규칙 무변경**.
    1차의 A1·A2b 는 누수 없는 층이라 유효하고 2차와 거의 같다.
  · **한계**: 모집단이 이미 선별을 거친 목록 · 국면 홀드아웃이 bear 지배 · **승률과 청산이 얽혀 있다**(방식D −8% 손절이
    승률을 구조적으로 누른다 — 청산을 바꾸면 답도 바뀐다) · A3 두 셀이 같은 패턴.
  · **문턱 완화·유지는 사용자 결정** — 증거는 한쪽으로 기울었으나 판정은 INCONCLUSIVE 이고 CLAUDE.md 상 게이트 문턱
    변경은 사용자 결정 칸이다. audit_winrate.py / test_audit_winrate.py(36) / winrate_audit.yml
  · **사용자 결정 (2026-09-15) — 35% 유지, 10/06 이후 '승률 → 복권형 지표 대체' 설계로 간다.**
    묻는 형태가 '30% 로 낮춰도 되나' 였고 답은 **낮출 수는 있으나 지금은 아니다**:
    · **≥30% 와 ≥25% 가 완전히 동일**(둘 다 11셀 +5.84%) — 이 모집단의 최저 승률이 31% 라 **30% 로 낮추는 것은
      '승률 조건을 없애는 것' 과 결과가 같다.** 진짜 질문은 35 대 30 이 아니라 '승률 조건을 쓸 것인가' 다.
    · 낮추면 들어오는 것은 **정확히 2셀, 둘 다 `triple_bottom_1d`**(bull_btc 승률 31% 홀드아웃 +20.29% n=53 /
      bear 34% +3.44% n=34). 둘 다 **top5 기여 41%** — 문턱이 잡으려던 복권형 그 자체이고, 이 패턴은 방식D 에서 **62% 가 손절**이다.
    · **9/12 보고 정정 — '증거 세 줄기' 는 사실 한 관찰이다.** A2 의 최고 버킷(30~35%)·A3 거짓음성·A4 가
      ≥35%→≥30% 에서 얻는 이득이 **전부 이 2셀**이고, 둘은 같은 패턴의 레짐 분해라 독립 증거는 1건이다.
      넓은 지지가 있는 쪽은 따로다 — **'승률 40%+ 구간이 가장 나쁘다'**(A2b 10셀 +1.47%). 승률을 *높게* 요구할
      근거가 없다는 뜻이지 30% 까지 열라는 근거는 아니다.
    · 미루는 이유 둘 더: **관찰 기간(~10/06) 중** 배포 집합을 늘리면 대조표가 오염된다(9/06 '더 이상 끄지 않는다' 의 거울상) ·
      손절 잦은 1d 패턴 추가의 **포트폴리오 효과 미측정**(9/10 프레임은 배분 규칙만 쟀고 패턴 추가는 안 쟀다).
  · **후속(10/06 이후 사전 등록)**: 문턱을 옮기는 대신 **승률을 top5 기여도·절사평균갭으로 직접 대체**. 근거는 A1 —
    rho(승률,Calmar) −0.183(무관) / rho(승률,절사갭) +0.786 / rho(승률,top5) −0.573. 승률은 복권형의 대리 변수일
    뿐이라 직접 재면 더 정확하다. 두 지표는 이미 게이트 안에 **진단으로** 들어 있어 판정으로 승격만 하면 된다.
...
- **사용자 강제 실행 — tp1_engulfing_1h, $30 (2026-09-09, 사용자 지시 "그냥 30달러만 강제 진행해볼수 있어? 지금 btc 들어가있는거 청산하고")**:
  ...
  · **손절 바닥 $70 (2026-09-19 같은 날 재지시 "손절 금액이 70달러 미만이면 다시 70으로 리셋, 70달러 위면 그냥 유지")**:
    registry `live_cap.loss_floor` **true**, `reset_on_loss` false. `paper_executor.cap_margin` 규칙 —
    익절·손절 모두 곱셈 누적(포트 × (1+ret×3))하되, **손절로 포트가 $70 아래로 떨어지면 $70 으로 올린다.**
    위에서 깎인 손실은 그대로 유지(예: $84.60 −1% → $82.06 유지 / $84.60 −8.2% → $63.79 → **$70**).
    · 몇 시간 전의 '리셋 제거'(reset_on_loss false 만, 손실 전부 이어받기)는 이 지시로 **철회** — 그 판이라면
      AAVE 손절로 $63.67 이 됐을 것. 바닥 규칙에선 AAVE(76.70×0.7546=57.88<70)가 $70 으로 올라가 종전 리셋과 같은
      결과 → 현 포트 **약 $77.00 유지**(승4 누적배수 1.100). 다음 tp1 `cap_margin` 로그로 확인.
    · 세 규칙이 코드에 공존한다 — `reset_on_loss`(무조건 $70) / 순수 복리(둘 다 false) / **`loss_floor`(현행)**.
      `loss_floor` 와 `reset_on_loss` 가 갈리는 건 포트가 $70 위에서 작은 손실을 맞을 때뿐(익절 +1%/손절 −8% 배리어라
      실제로는 손절 = −8.2%×3 = −24.6% 이고, 포트가 $70×1/0.754 = **$92.8 을 넘어야** 손절 뒤에도 $70 위에 남는다 —
      익절 12연승이 필요). 즉 당분간 바닥 규칙은 종전 리셋과 사실상 같은 값을 낸다.
    · test_tp1_live §7 — 실측 AAVE 재현 → $70 · 큰 포트 −1% 유지 · 큰 포트 손절 → $70 · 세 모드 분기 · 순서 무관.
  · **손절 컷 스윕 사전 등록 (2026-09-19, 사용자 질문 "1년 동안 이 3배 레버리지 기준으로 손절컷을 8%로 잡는데 최대이익을
    달성할수 있는 손절컷 맞는지")**: validate_tp1_stop.py / test_tp1_stop.py(23) / tp1_stop.yml. **DEPLOY_ON_PASS=False — 손절 변경은
    사용자 결정.** 익절 1%·3x·순차 1포지션·바닥 $70·top20(30봉 거래대금)·engulfing 1h 인자 없음 — 실거래 규칙 그대로 두고 **손절 %만
    {1,2,3,4,5,6,7,8,10,12,15,20}% 스윕**. 판정량은 계좌 손익 $(Σ 증거금×ret×3, 1년 1h 데이터) 이고 8% 대비 차이·전후반 부호·
    손절 수·포트 최종값을 병기. 배리어 판정은 tp_1h 와 같이 신호 다음 봉부터, 같은 봉 동률은 손절 우선(보수). 포트 스텝은
    `pe.cap_margin` 과 12스텝 일치를 테스트가 고정. **이 환경은 OKX 시세 차단이라 러너에서 실행**(push 가 워크플로를 깨운다).
    사전 확률(결과 전): tp_1h 격자가 손절 4% 최악·20% 까지 개선·무손절 악화였고 승률 88% < 손익분기 91% 라 **전 셀 음수** 유력 —
    '최대이익' 이 아니라 '최소손실' 손절을 찾는 판이 될 가능성이 높다. 결과는 아래 항목에 기록.
    · **결과 (run 35439231280) — 24셀 전부 음수, 8% 유지(변경 근거 없음)**: registry `tp1_stop_prereg_2026_09_19`.
      top20(신호 2,204 → 순차 체결 126~694) 계좌손익 $: 1% −367 / 2% −255 / 3% −258 / 4% −202 / 5% −171 / 6% −129 / **7% −81** /
      **8%(현행) −149** / 10% −93 / **12% −80(최소손실)** / 15% −102 / 20% −243. 승률은 전 셀 손익분기 아래(8%: 89.2% vs 91.1%).
      · **8% 는 top20 에서 최소손실이 아니지만(12% 대비 −$69) 격자가 비단조** — 7% 와 12% 가 같은 값이고 그 사이 8% 가 골이다.
        손절 30건 안팎이 결과를 정하는 표본이라 잡음이고, **all 80 코호트에서는 8% 가 최소손실**(−169)이며 12% 는 −267 로
        더 나쁘다. 코호트를 바꾸면 순위가 뒤집히므로 손절을 옮길 근거가 없다.
      · 넓은 손절(10~20%)의 후반 양수(+28~+112)는 2026-03~09 한 구간이고 전반이 더 깊은 음수 — 단일 구간 의존(켜지 않는 원칙).
      · **포트 최종값(12% $203·15% $234)은 계좌 손익이 아니다** — 바닥 규칙이 손절 손실을 계좌로 흡수하고 포트는 $70 으로 돌아가
        승만 복리로 쌓이기 때문. 판정량은 계좌 손익 $ 하나.
      · 사전 확률 적중(전 셀 음수). tp_1h/tp_regime 의 '이 규칙은 어느 배리어에서도 수수료를 못 넘는다' 재확인. **실거래 무변경.**
      · 한계: 정적 top20 · 봉 안 순서 미관측(동률 손절 우선 = 보수 하한) · 신호봉 종가 진입 가정 · 1년 단일 bear 창.
  · **익절 × 손절 2차원 격자 사전 등록 (2026-09-21, 사용자 질문 "익절을 2%로 올리면 손익분기가" → 지시 "익절 축 추가해서 1h로 돌려줘")**:
    validate_tp1_grid.py / test_tp1_grid.py(39) / tp1_grid.yml / registry `tp1_grid_prereg_2026_09_21`. **DEPLOY_ON_PASS=False.**
    · **먼저 답한 계산**: 손익분기 =(SL+수수료)/(TP+SL) 이라 익절 1%→2% 면 **91.11%→82.00%**. 그러나
      랜덤워크 도달률도 88.89%→80.00% 로 같이 내려가 **필요 리프트는 2.22%p→2.00%p** 로 거의 안 변한다.
    · **폐형**: `EV = −δ × (TP+SL) − 수수료` (δ = 랜덤워크 대비 실측 승률 부족분). **수수료 항이 TP 와 무관하게
      고정**이라, δ 가 일정하면 익절을 넓혀도 건당은 오히려 미세하게 나빠진다(−0.280/−0.289/−0.298%).
      → **이 판의 실제 질문은 δ(TP) 가 변하는가 하나**이고 그건 계산이 아니라 측정이다.
    · **구멍**: tp_1h 격자는 전부 익절 1%(T1S3/T1S8/T1S20/T1SX)였고 익절 2%·3% 는 1d 판(tp_small)에만 있었는데
      그건 봉 안 동률 오염이 심했다 — **1h 에서 익절 축을 잰 적이 없다.**
    · 동결: 익절 {1,1.5,2,2.5,3}% × 손절 {1,2,3,4,5,6,7,8,10,12,15,20}% = **60셀** · top20 주 판정/all 진단 ·
      1h 365일 · 나머지 규칙(3x·순차 1포지션·바닥 $70·동률 손절 우선·2000봉)은 어제 판과 동일 ·
      **랜덤 진입 베이스라인 3000건(시드 고정)** 으로 δ 직접 측정 · 판정량은 계좌 손익 $.
    · **방법**: 60셀이라 신호당 전방 스캔 1회로 전 임계값 최초 도달 봉을 구하는 최적화(`first_hits`/`resolve_hits`,
      임계값 오름차순 포인터 1개 + import 시 assert). **test_tp1_grid 가 합성 7,200셀 + 극단 경로 6종에서
      어제 판 `validate_tp1_stop.resolve` 와 완전 일치를 고정** — 어긋나면 수치 전부가 무의미하므로 이게 근간.
    · 사전 확률(공개): 위 폐형과 δ=0.89%p 대입값을 **실행 전 대화에서 이미 계산해 보고했다**(눈 감은 예측 아님).
      그 가정대로면 60셀 전부 음수. 단 계좌 손익은 건당×거래수라 순위가 뒤집힐 수 있어 예측하지 않는다.
    · **결과 (run 35562250210) — 120셀 중 양수 2셀뿐, 익절 1% 유지(변경 없음)**: registry `tp1_grid_prereg_2026_09_21`.
      top20 신호 2,195(순차 체결 116~288) / all 7,933.
      · **(b) 질문 직답 — 익절 2% 는 더 나쁘다.** top20 2%/8% **$-169.23** vs 현행 1%/8% **$-125.96**(−$43.27) ·
        all $-308.00 vs $-141.72(−$166.28). 두 코호트 동부호. 손익분기가 91.11%→82.00% 로 9%p 내려가지만
        **실측 승률이 89.58%→77.59% 로 12%p 더 내려간다** — 분기점까지 부족분이 −1.53%p → −4.41%p 로 오히려 벌어진다.
      · **(c) δ 는 익절에 따라 거의 안 변한다 — 폐형 전제 적중.** 익절별 평균 δ(%p) top20 +2.97/+3.13/+3.11/+2.92/+2.63,
        all +1.25/+1.73/+1.52/+1.70/+1.55. 단조도 아니고 넓힌다고 줄지 않는다.
      · **새 정보 — 무너지는 건 δ 가 아니라 패턴 엣지다.** 패턴 승률 − 랜덤 승률(top20 손절8%): 익절 1.0% **+3.35%p** /
        1.5% +0.77 / 2.0% +0.55 / 2.5% **−1.36** / 3.0% −1.10 (all 도 +1.74/−0.75/−4.19/−3.88/−1.49 로 같은 모양).
        **engulfing 1h 신호는 좁고 빠른 반등이지 지속 상승이 아니다** — 익절을 넓히면 그 엣지를 못 받고 랜덤보다 나빠진다.
        폐형은 맞았고 틀린 가정은 'δ 일정' 이 아니라 **'패턴 부족분 일정'** 이었다(EV 식 대입 −0.14/−0.44% vs 실측 −0.11/−0.40%).
      · **현행 1% 가 익절 축에서 최선이고 분기점에 가장 가깝다** — 실측 89.58% 는 **이론 도달률 88.89% 보다 높다**
        (패턴이 랜덤워크를 이긴다). 남은 음수 **건당 −0.11%** 는 사실상 왕복 수수료 0.2% 다. 넓힐수록 −0.38/−0.40/−0.58/−0.55%.
      · (a) 계좌손익 최대 셀 top20 1.5%/12% **$+1.63** · all 1.5%/15% **$+29.15** — 코호트마다 다른 셀이고
        **전후반 둘 다 양수인 셀은 0**. 사후에 고르지 않는다.
      · 사전 확률: '전 셀 음수'·'건당은 넓을수록 나쁘다'·'δ 일정' 적중. **'보유가 길어 거래가 줄어 총손실도 줄 수 있다' 는
        안 맞았다** — n 288→116 으로 줄어도 건당 악화가 앞선다(보유 21.3→65.6h).
      · 손절 8% 슬라이스 **동률 0.0%** — 넓은 손절에서는 봉 안 순서 오염이 없다(좁은 손절 셀은 여전히 보수 하한).
      · 한계: 정적 top20 · 1h 365일 단일 bear 창 · 신호봉 종가 진입 가정 · 순차 1포지션이라 신호 대부분을 버린다.
      **익절·손절 변경 없음(1%/8% 유지), 실거래 무변경.**
  · **max_open 2·3 + 수익 중 교체 규칙 사전 등록 (2026-09-21, 사용자 지시 "맥스오픈 2,3 로 올리면 어떻게 되는지
    테스트" / "tp1 진행중일때 다른 신호 뜰경우 수익 구간 중이면 바로 익절 날리고 새로운 신호로 교체하면 어떻게 되는지 테스트")**:
    validate_tp1_slots.py / test_tp1_slots.py(40) / tp1_slots.yml / registry `tp1_slots_prereg_2026_09_21`. **DEPLOY_ON_PASS=False.**
    · **계기 — 실측**: fast 로그 100틱(9/17 02:03Z~9/21 05:03Z, 4.1일) 전수. tp1 **신호 13건 중 10건이 max_open=1 캡에
      막혀 스킵**(체결 3건 = **0.73/일**, 신호 3.2/일). 어제 격자의 순차 체결 288건/년 = 0.79/일과 일치 — 체결 빈도는
      검증값대로 나오고 있고, **막히는 신호 77% 는 한 번도 측정된 적이 없다.**
    · **중요한 모델링 사실**: `cap_margin` 은 슬롯 수로 증거금을 나누지 않는다 — 진입마다 **포트 전액**이다.
      따라서 max_open=N 은 곧 **최대 노출 N 배**이고, 건당이 음수인 규칙에서 '슬롯 2·3 이 더 잃는다'는 거의 자명하다.
      그 자명함을 걷어내려고 **노출 정합 arm(split, 증거금 = 포트/N)** 을 병기한다 — 남는 잔차가 '막힌 신호의 질'이다.
    · 동결: 익절 1%·손절 8%·3x·포트 바닥 $70·top20·같은 종목 중복 금지 전부 실거래 그대로. arm = max_open {1,2,3}
      주판정 + {4,6,8} 진단 × 교체문턱 {없음, **0.0%**} 주판정 + {0.2%, 0.5%} 진단 × {full, split} = **48셀**.
      판정량은 계좌 손익 $.
    · **교체 규칙 정의**: 슬롯이 찬 상태에서 새 신호 → 열린 포지션 중 그 시각 종가 기준 **총수익**이 문턱 초과면
      **가장 수익 큰 것**을 그 종가에 청산(수수료 차감)하고 교체. 이번 틱 진입분은 대상 아님.
    · **사전 확률(결과 전)**: full 에서 슬롯 2·3 은 1 보다 나쁠 것. **교체 규칙도 나쁠 것** — 기전 둘: (a) 이기는
      포지션만 잘라 '승자를 짧게·패자를 길게' (b) 문턱 0% 는 총수익 0~0.2% 구간에서 **순손실을 확정**한다
      (왕복 수수료 0.2%) — 대표님 표현의 '익절'이 실제로는 익절이 아닌 구간이 있다. 반대 방향 여지는 회전율뿐.
      크기·순위는 예측하지 않는다.
    · **기준선 동치가 근간** — 현행 셀 (1,없음,full)이 어제 판 `validate_tp1_stop.simulate(sl=0.08)` 과 거래·손익·
      스킵 수까지 완전히 같아야 한다(test §1 난수 40판 고정). 상수·resolve·pot_next 는 어제 판을 그대로 import.
    · **실데이터 스모크 없음** — 이 환경은 OKX 가 막혀 있고 로컬 부분 실행은 결과를 미리 보게 된다(yyy_cross 전례).
    · **결과 (run 35565593363) — 96셀(48×2코호트) 양수 0. max_open 1 · 교체 없음 유지, 실거래 무변경**:
      registry `tp1_slots_prereg_2026_09_21`. top20 신호 2,195 / all 7,933.
      · **(a) 슬롯 2·3 은 명확히 나쁘다.** top20 계좌손익 $: 1(현행) **−126** / 2 **−396** / 3 **−824** / 4 −1,322 / 6 −2,801 /
        8 −4,973. **노출 정합(split)으로도 여전히 나쁨** — −126 / −198 / −275. '증거금이 N 배라서' 만이 아니다.
      · **(c) 새 정보 — 막히던 신호 77% 는 들어오는 신호보다 나쁘다.** 건당이 슬롯 수에 **단조 감소**한다:
        top20 **−0.11%**(슬롯1) → −0.18 → −0.22 → −0.23 → −0.27 → **−0.29%**(슬롯8), 승률 89.58% → 87.81%
        (all 도 −0.13 → −0.30% 로 동형). 기전 추정: 신호가 **버스트로 몰리는 시각**(시장 전체 급락)의 결과가 나쁜데
        슬롯이 많을수록 그 버스트를 통째로 받는다(시간가중 동시보유 0.70 → 3.47). **슬롯 1 이 사실상 '버스트 필터'로
        작동하고 있었다.**
      · **(b) 교체 규칙도 나쁘다** — top20 $−137 vs −126, 승률 89.58%→**83.84%** / all $−255 vs −142, 승률 89.38%→**80.19%**.
        두 코호트 동부호.
      · **사전 확률에 적은 기전 둘이 그대로 관측됐다**: ① **승자만 잘린다** — 익절 258→216(−42)인데 손절은 29→28 로 불변.
        +0.8% 갈 거래를 **+0.12%**(교체 건당 실측)에 끊는다. ② **문턱 0% 는 순손실을 확정한다** — 297거래 249승인데
        익절이 216건이니 교체 52건 중 **33승 19패**. 총수익 0~0.2% 구간이 왕복 수수료에 잡힌 것으로, '익절'이 익절이
        아닌 구간이 **실제로 19건** 있었다. 회전은 빨라졌으나(all 보유 23.9→17.9h) 보상 못 함.
      · **진단 문턱 0.2%(순수익 0)는 top20 슬롯1 에서 $−89.75 로 격자 최대**(승률 90.72%, 건당 −0.08%)이나 **채택 후보 아님** —
        진단 셀이고, **all 코호트에서 부호가 뒤집히며**(−185 vs 현행 −142), 전후반 둘 다 양수가 아니고, 여전히 음수다. 사후 선택 안 함.
      · **기준선 동치 러너 확인** — 현행 셀 $−126.12 · 건당 −0.11% · n=288 이 어제 격자의 같은 셀($−125.96 · −0.11% · n=288)과
        일치($0.16 차는 1h 봉 1개 차).
      · 사전 확률 **전부 적중**(슬롯 열세 · 교체 열세 · 기전 ①② · split 잔차 = 신호 질). 크기·순위는 예측하지 않았다.
      · 한계: 정적 top20 · 1h 365일 단일 bear 창 · 같은 봉 신호 순서는 입력순 · 실거래에서 슬롯 2·3 은 증거금($81×N vs free $243)도 별도 제약.
      **max_open·교체 규칙 변경 없음(1 · 교체 없음 유지), 실거래 무변경.**
  · **BTC 제외 (2026-09-22, 사용자 지시 "tp1 비트 포지션 내가 방금 수동 청산했어. 당분간 tp1 은 비트 제외하고
    내가 말할 때까지 다른 나머지걸로만")**: registry `tp1_engulfing_1h.symbol_exclusion_2026_09_22`.
    universe.json 의 tp1 항목에 `exclude: ["BTC"]`, scheduler `_cohort_symbols(rule, base, exclude)` 가
    **코호트를 적용한 뒤** 그 심볼을 뺀다.
    · **코호트 뒤에 빼는 이유** — 순위 계산에서 BTC 를 빼면 21위가 올라와 'top20' 의 뜻이 달라진다.
      결과에서만 제외하면 나머지 19종목은 종전과 완전히 같다(test §8 이 top3 예시로 고정).
    · **`exclude` 는 선택 필드 — 없는 항목은 한 줄도 안 바뀐다**: cascade_fade_long_1h · 4h adopted 3종 ·
      1d adopted 전부 불변(test_tp1_live §8 · test_adopted_regimes 가 '현 배포 항목에 exclude 없음' 고정).
      사이징·배리어·슬롯·중복 방어·청산 규칙 전부 불변.
    · **수동 청산은 엔진 조치 불필요** — 다음 fast 틱에서 reconcile 이 포지션 소멸을 보고 D 청산을 기록하고,
      `ensure_stop_orders` 고아 취소가 남은 OCO algo 를 지운다(reduceOnly 라 그 사이에도 신규 진입 위험 없음).
      사유는 `exit_reason_of` 가 체결가를 배리어와 대조해 **'OKX청산'**(수동·외부)으로 붙인다 — 손절로 안 센다.
    · **단 복리 포트에는 들어간다** — `cap_margin` 이 이 패턴의 D 청산 거래 전부를 곱하기 때문(바닥 $70 불변).
      규칙이 낸 결과가 아니므로 **10/06 대조 보고에서 이 1건은 따로 표시**하고 승률·건당 집계에서 구분한다.
    · 되돌리기: universe.json tp1 항목의 `exclude` 삭제 — **사용자 지시가 있을 때만.**
  · **시작 증거금 $30 → $70 (2026-09-10, 사용자 지시 "증거금 367.46 USDT 추가입금했어 … 기본 포지션 스타트 금액 70으로 상향")**:
    ...
    명목 $210(3x). 규칙·경로 무변경, 금액만.
    포트는 거래 이력에서 매 실행 재계산하므로 상향 전 거래가 있어도 새 시작값 70 에 곱해진다.
    · **정정 (2026-09-21) — 'ETH·BTC 는 계약 단위 때문에 스킵' 은 틀렸다.** 둘 다 실제로 진입했다:
      ETH 9/18 14:03Z 증거금 $75.19 → 명목 **$225.57** · BTC 9/21 01:03Z $80.74 → **$242.22**.
      `qty_below_lot_min` 은 계약 수 기준(`contractSize` 환산 후 `limits.amount.min`)이라 BTC/ETH 무기한의
      최소 계약이 이 명목가 안에 들어온다. **fast 로그 100틱(4.1일) 전수에서 lot·잔고 스킵 0건.**
  · **시작 증거금 $70 vs $500 비교 (2026-09-21, 사용자 질문)**: **시뮬은 시작 증거금에 완전 선형이다** —
    `pot = max(pot×(1+ret×3), start)` 와 `계좌손익 = Σ 증거금×ret×3` 이 둘 다 start 에 1차 동차라, 합성
    데이터 3 arm 에서 배수가 **7.142857(=500/70) 정확히 일치**하고 거래 집합·승률·건당이 완전히 같다.
    즉 $500 판은 $70 판 수치에 7.14 를 곱한 것이고 **새 정보가 없다**(러너 재실행 불필요).
    · top20 1년: 계좌손익 **−$126 → −$901** · 실현 MDD −$179 → −$1,278 · 포트 최종 $101 → $724.
      거래 288건·승률 89.58%·건당 −0.11%·보유 21.3h 는 **양쪽 동일**.
    · **척도 불변 표현**: 이 규칙은 **1년에 시작 증거금의 약 1.8배**를 계좌에서 태운다(top20 126/70 = 1.80,
      all 142/70 = 2.03). 바닥 규칙이 손절 뒤 포트를 start 로 되돌리므로 **손실이 시작 증거금을 넘을 수 있다** —
      시작금을 키워도 비율은 그대로다.
    · **선형성을 깰 수 있는 경로 둘, 둘 다 지금은 비활성**: ① lot 최소 — 위 정정대로 0건 ② 최소주문 $10 —
      바닥이 $70 이라 닿지 않는다. 실거래에서 유일하게 다른 것은 **가용잔고**다: `place_swap_entry` 는 free 가
      모자라면 스킵이 아니라 **free×0.9 로 축소**하므로(exchange.py:456), 현 계좌(free 약 $243)에서 $500 은
      그대로 안 나가고 약 $219 로 줄어든다 — **$500 시작은 현 계좌에서 실행 불가**(충전 또는 메인 포지션 감소 선결).
...
- **레짐 전환 청산이 꺼진 채 돌던 연구 2건 — 결함 발견·수정 (2026-09-21)**: registry
  `regime_exit_off_defect_2026_09_21`. ⑤ 프레임을 세우다 발견. `method_t.outcome_d` 는 방식D 의
  **레짐 전환 청산**을 `REGMAP.get(date) not in (None, entry_reg)` 로 판정하는데, 모듈 기본값이
  `REGMAP = {}` 이라 호출자가 안 채우면 `.get()` 이 항상 None → **조건이 항상 False → 레짐 전환
  청산이 통째로 사라진다.** 예외가 아니라 조용한 무력화다.
  · **실증**: 합성 40봉·5봉째 진입·8봉째 레짐 전환에서 빈 맵이면 `maxhold`(30봉), 채우면
    `regime_switch`(3봉). 같은 신호가 전혀 다른 거래가 된다.
  · **영향받은 것 둘**: **sizing_vol.py**(2026-09-04 변동성 타겟팅 **채택** 연구 + 레버리지·위험
    격자 + MAX_POS 슬롯 격자) · **validate_portfolio.py**(2026-09-10 슬롯 배분, run 34432328827).
  · **나머지는 전부 멀쩡하다** — method_r/method_x/method_m/method_q/regime_axis/sizing_study/
    validate_ih_exit/validate_tp_small/method_d/candle_verify/tf_verify 는 모두 `mt.REGMAP =
    rs.build_regime_map()` 를 명시한다. **일관된 관례에서 둘만 벗어났으므로 설계가 아니라 누락이다.**
  · **판정은 살아남을 가능성이 높고 절대 수준은 아니다** — 두 연구 다 arm 을 같은 거래 집합 위에서
    비교했으므로 우열(사이징 arm, 슬롯 arm 전부 기각)은 상쇄된다. 그러나 CAGR·MDD·Calmar·P(ruin)·
    보유기간·청산 사유 분포는 실거래 규칙이 아니었다. 레짐 전환은 실거래 eval_D 에서 **fvg 청산의
    약 1/3 을 담당하는 주 출구**다. **채택된 상수 자체는 무관** — VOL_S_NORM 1.1094 는 신호 시점 σ 의
    평균이라 청산 규칙과 독립이다.
  · **수정**: 두 main 에 한 줄 추가 + `method_t.regmap_ready()` 공개 + outcome_d 첫 호출 시
    **1회성 stderr 경고**(동작 무변경). test_port_vol §6 이 경고·무력화 사실·세 모듈의 설정을 고정.
  · **우발적 재실행 (2026-09-21) — 수정 커밋이 두 모듈의 경로 트리거를 건드려 저절로 돌았다. 사전 등록된
    재판정이 아니다.** 결과가 두 연구에서 갈렸다:
    · **sizing_vol (run 35572055349) — 판정 유지.** 전 표본(2,780일·부트 300) 4조건 전부 통과
      (boot Calmar 0.68 vs 0.41 / MDD −81.1 vs −84.5% / P(ruin) 4.7 vs 27.7% / 노출 0.95배),
      라우팅 복제판(n=1,065)도 4/4(2.05 vs 1.54 / −51.5 vs −57.7% / 0.7 vs 1.3% / 0.93배).
      **변동성 타겟팅 채택 근거는 결함을 고쳐도 그대로다.**
    · **validate_portfolio (run 35572055410) — 판정이 뒤집혔다.** holdout Calmar current **−0.81** vs
      **prio_edge −0.57** / cap4 −0.60 / cap8 −0.60 / cohort20 −0.63 / cap6 −0.82 → 출력 `후보 prio_edge`.
      2026-09-10 에는 current −0.63 이 최선이고 5 arm 전부 열세였다.
    · **내가 '판정은 살아남을 가능성이 높다'고 적은 것은 틀렸다** — 슬롯 arm 비교는 상쇄되지 않았다.
      청산 규칙이 보유기간을 바꾸면 **슬롯 점유 시간**이 바뀌고 슬롯 배분 연구는 바로 그것을 재기 때문에
      '같은 거래 집합' 논리가 이 연구에는 성립하지 않는다. 사이징 arm(명목가만 바꿈)에는 성립했다.
    · **원인 귀속 불가(교란)** — 9/10 대비 세 가지가 동시에 다르다: REGMAP 수정 + MAX_POS 16→13 +
      RISK_FRAC 1.0%→1.5%(뒤 둘은 2026-09-18 사용자 결정). 가르려면 한 번에 하나씩 돌려야 한다.
    · **실거래 무변경** — 슬롯 배분은 위험 배분이라 사용자 결정이고 DEPLOY_ON_PASS=False, 관찰 기간 중.
  · **실거래 영향 없음**(실거래 청산은 paper_executor.eval_D 이고 그쪽 레짐맵은 정상).
- **펀딩비 손익 반영 사전 등록 (2026-09-21, 사용자 지시 "2,3,5 진행" — ②)**: registry
  `funding_cost_prereg_2026_09_21`. **이 레포의 어떤 손익 계산에도 펀딩비가 들어간 적이 없다**
  (확인: method_x/method_t/validate_revival/paper_executor/intraday_lab/exit_barriers/sizing/detlib
  전부 0건). 비용은 `intraday_lab.FEE = 0.002`(왕복 0.2%) 하나뿐인데 배포 집합은 **전부 알트 무기한
  롱**이고 방식D 는 최대 30봉(1d 30일 / 4h 5일)을 든다. **새 엣지가 아니라 기존 엣지의 진짜 크기**를 잰다.
  · 소스 OKX 1순위, Bybit 는 겹치는 창에서 ρ≥0.80 AND 평균차≤0.005%/8h 일 때만 확장. 부과는
    (t_in, t_out] 정산 요율 합 × 방향부호. 거래 집합은 validate_portfolio 배포 정의 그대로.
  · 판정 **MATERIAL / IMMATERIAL** — (i) 평균 비용 ≥ 평균 총수익의 20% (ii) 셀 부호 역전
    (iii) 포트폴리오 Calmar 10% 하락. DEPLOY_ON_PASS=False, 어떤 결과든 실거래 무변경.
  · 사전 확률: 평시 0.01%/8h 면 1d 약 0.3% / 4h 약 0.15% → **vol_awakening_4h(OOS +0.21%)는 뒤집힐 수
    있고** triple_bottom_4h(+1.59%)는 안 뒤집힌다고 본다. **측정값은 하한일 가능성이 높다** — OKX 창
    (약 3개월)이 bear 지배라 펀딩이 낮다. IMMATERIAL 이어도 '불장에서도 작다'는 뜻이 아니다.
  · 커버 안 되는 과거 거래는 추정치로 채우지 않고 제외한다. validate_funding_cost.py / test(50건) / funding_cost.yml
  · **결과 MATERIAL — 단 세 조건 중 가장 약한 하나로만 (run 35572055378). 실거래 무변경**:
    펀딩 **건당 0.073%p**, 평균 총수익 +1.68% 의 **4.3%** → (i) 20% 미달. **셀 부호 역전 0** → (ii) 미달.
    **(iii) 포트폴리오 Calmar 25.52 → 21.99(−14%)** 하나로 MATERIAL.
    · **(iii) 는 과장 쪽이다** — 커버 창이 96일(2026-06-17~09-21)이라 CAGR 연율화 지수가 약 3.8 배다.
      최종 자산 차이는 CAGR 차이(590→527%)가 시사하는 것보다 훨씬 작고, Calmar 25.52 라는 절대값 자체가
      짧은 창의 산물이다 — **실거래 예보로 읽지 말 것.**
    · **요율이 사전 확률을 확인했다** — 일당 4h 0.0183%p / 1d 0.0125%p ≈ **0.004~0.006%/8h** 로 평시 가정
      0.01%/8h 의 절반 이하. 커버 창이 bear 지배라 펀딩이 낮았고 **이 측정은 하한**이다.
    · **커버리지 9.7%(851/8,817)** — OKX 이력이 약 3개월. **Bybit 확장은 0건 회수**(겹침 0 → 게이트 미적용);
      원인은 로그로 안 갈린다(지오블록/파라미터/삼킨 예외). 확장이 됐으면 창이 몇 년으로 늘었을 자리다.
    · **사전 확률 오답 하나** — vol_awakening_4h 가 뒤집힐 거라 봤으나 커버 창 건당 +1.04% 라 여유가 컸다
      (+1.04%→+0.96%). 커버 창에 **bull_btc 가 없어** 원래 묻고 싶던 '알트 강세에서 펀딩이 튀는가' 는 미측정.
    · **읽는 법**: 펀딩은 실재하나 현 요율에서 엣지의 약 4%. 위험은 평균이 아니라 꼬리 — 알트 강세에서
      요율이 5~20배 뛰면 얇은 셀(vol_awakening_4h OOS +0.21%)은 뒤집힌다. 잴 인프라는 이제 있다(perp_daily).
- **형성 중인 봉 탐지 사전 등록 (2026-09-21, 사용자 지시 — ③)**: registry
  `forming_bar_prereg_2026_09_21`. CLAUDE.md '미해소 gap(의도적)' 의 소진. 배포 패턴 일부는 아직
  `rows[-1]`(형성 중인 봉)에서 탐지하는데 검증은 닫힌 봉 기준이고, **그 불일치의 크기가 한 번도
  측정된 적이 없다**(같은 종류의 결함으로 하모닉 5종이 등재 정지).
  · 셀 7 = 1d engulfing 롱·숏 / fvg 롱·숏 / inverted_hammer / marubozu · 4h three_soldiers_4h.
    **레짐 라우팅 미적용(무조건부)** — 신호 집합 차이는 라우팅과 무관하고 걸면 검정력만 준다.
    제외는 detect_on_closed_bar 를 가진 4h 3종과 `_closed_idx` 를 쓰는 cascade·tp1·adopted_1h.
  · 합성: 1d 부분봉은 **4h 로**, 4h 부분봉은 **1h 로**(OHLCV 합산은 그 구간 OHLC 와 정확히 같다).
    틱 (0,4,8,12,16,20) UTC, h=0 은 **빈 봉**. 한 봉에 첫 발화 한 번만(중복 키가 봉 ts).
  · 판정 MATERIAL — (i) only_forming ≥20% (ii) 건당 1.0%p 이상 하락 (iii) forming 건당 음수.
  · **사전 확률: three_soldiers_4h 가 가장 위험하다** — 4h 는 틱과 봉 경계가 일치해 빈 봉에서
    탐지하므로 3연속 장대 양봉이 성립할 방법이 사실상 없다(하모닉과 같은 '진입 0건' 구조일 수 있다).
    1d 는 반대로 **몸통을 요구하는 engulfing·marubozu 는 덜 발화**하고 **위꼬리를 요구하는 ih 는 더
    발화**할 것으로 본다. 진입가는 유불리가 갈리지 않고 **문제는 신호 집합**이라고 본다.
  · **결과 MATERIAL (run 35572055387) — 이번 세 건 중 가장 큰 발견. 실거래 무변경**:
    | 셀 | closed | forming | only_f | only_c | only_f% | closed 건당 | forming 건당 |
    |---|---|---|---|---|---|---|---|
    | engulfing | 184 | 67 | 9 | **126** | 13% | +1.80% | +1.90% |
    | engulfing_short | 245 | 130 | 38 | **153** | 29% | +0.60% | **−0.19%** |
    | fvg | 1666 | 893 | 54 | **827** | 6% | +4.07% | +7.81% |
    | fvg_short | 1329 | 627 | 21 | **723** | 3% | −0.06% | **−1.01%** |
    | inverted_hammer | 107 | 345 | **320** | 82 | **93%** | −0.54% | +5.26% |
    | marubozu | 21 | 51 | **47** | 17 | **92%** | +9.99% | **+3.97%** |
    | three_soldiers_4h | 369 | **0** | 0 | 369 | — | +1.51% | — |
    · **판정 조건보다 only_closed 가 크다 — 검증 신호의 절반 이상이 실거래에서 영구 누락된다.**
      fvg 50% · engulfing 68% · fvg_short 54% · engulfing_short 62%. 형성 중인 봉에서 못 잡으면 다음 틱엔
      그 봉이 `rows[-2]` 라 **영영 못 잡는다.** '실거래 거래 수가 검증보다 적다'의 정량이 이것이다.
    · **ih·marubozu 는 검증한 것과 거의 다른 규칙을 돌린다** — 실거래 신호의 92~93% 가 닫힌 봉에 존재하지
      않는다. 사전 확률('위꼬리를 요구하는 ih 는 더 발화') 적중. 단 **ih 는 forming 이 더 좋다**(+5.26% vs
      −0.54%) — 손실의 증거가 아니라 **미검증**의 증거. marubozu 는 명백히 나쁘다(+9.99%→+3.97%).
    · **three_soldiers_4h — 내 사전 확률의 극단판은 실거래가 반증한다.** 빈 봉 모델에서 0건인 것은 맞지만
      실거래는 실제로 진입한다(9/15 GAS 손절, 9/09 EGLD 슬롯 스킵) — 러너가 탐지할 때 마지막 행이 완전히
      비어 있지 않다는 뜻이다. 진단 h1 판(첫 1h 봉까지)은 221건 **−2.44%**(closed +1.51%), only_forming 81%.
      **실제는 h0 과 h1 사이**이고 어느 쪽이든 **실거래가 잡는 집합은 검증된 집합이 아니다.**
    · 고치는 것(기존 패턴에 `detect_on_closed_bar` 를 켜는 것)은 **실거래 규칙 변경** — 사용자 결정 +
...
- **업비트 +50% 슈팅 전날 공통점 스터디 (2026-09-21, 사용자 지시 "업비트 최근 한 달 일봉 50% 이상 슈팅 코인 공통점")**:
  report_upbit_shooting.md / study_upbit_shooting.py (업비트 REST 직접, 이 환경에서 접근됨). **탐색적 — 배포 판정·사전 등록 아님.**
  창 2026-08-22~09-20 종가 +50% **11건**(고가만 도달 36건, 신규 상장 0), 베이스라인 코인-일 7,863.
  · **D−1 에서 갈린 것**: 20일 거래대금 34.9억 vs 11억(위 비율 0.82) · ATR/종가 9.4 vs 6.7%(0.82) · 20일 고저폭 53 vs 30%(0.82) ·
    D−1 거래량 2.48x vs 0.75x(0.73) · 60일 저점 대비 +50 vs +24%(0.73). **안 갈린 것**: MA180 위치·250일 고점 대비·이평선 위 개수·RSI.
    **캔들 패턴은 앞서지 않는다**(장악형·망치형 0%, 역배열 0%).
  · **두 유형 혼재** — A '달리던 중 2차 폭발' 7건(MA20 +33%, 20일 +51%, 볼폭 백분위 97%) / B '무거래 수축 뒤 급등' 4건(MA20 0%,
    거래량 0.9x, 볼폭 백분위 18%, 250고점 −72%). 스퀴즈 서사는 B 에만 맞고 A 는 정반대.
  · **슈팅 뒤 D+1 중앙 −13.5%, D+5 −19.4%(양수 14%)** — 당일 종가 추격은 손실. 당일 갭업 0%(장중 상승).
  · n=11 관찰이고 D−1 조건을 만족하는 코인-일은 베이스라인에 수백 건 — **역선택 불가**. 진입 규칙화는 별도 사전 등록. 실거래 무변경.
- **닫힌 봉 탐지 전환 사전 등록 (2026-09-21, 사용자 결정 "나로 진행해줘" — 준비만, 켜는 건 10/06 이후)**:
  ...
  **고쳤을 때 포트폴리오가 나아지는가**를 미리 잰다 — 관찰이 끝나는 날 바로 판단할 수 있게.
  ...
  · **결과 REJECTED (run 35577424571) — J1·J2·J3·J5 탈락, J4 만 통과. 실거래 무변경**:
    train current CAGR **+173.6%** Calmar **2.04** 체결 1033 vs closed_1d +73.8% **0.88** 체결 1233 ·
    holdout current −51.4% **−0.56** vs closed_1d −76.1% **−0.81** · 부트 우위 **33%**(문턱 60%).
    · **셀마다 방향이 반대다**(거래 수·건당): engulfing 59 +2.84% → **150 +3.39%**(둘 다 개선) /
      fvg 668 +10.55% → **1012 +6.49%**(수 1.5배·건당 절반) / **inverted_hammer 344 +5.28% → 107 −0.54%** /
      marubozu 51 +3.97% → 21 +9.99% / engulfing_short 32 +2.03% → 74 +0.80%.
    · **효과가 더해지지 않는다 — 슬롯 상호작용이다.** 진단 arm 을 따로 보면 closed_focus holdout Calmar
      **−0.56**(current 와 동률) · closed_adopted **−0.57** 로 둘 다 current 와 구분이 안 되는데, **합친
      closed_1d 은 −0.81** 로 뚜렷이 나쁘다. '어느 묶음이 손해다'가 아니라 두 전환이 **동시에** 걸릴 때
      슬롯 13 경합이 달라져서 나빠진다. 진단 셀이므로 사후에 'focus 만 켜자'로 가지 않는다.
      **패턴 단독 프레임이었으면 못 봤을 것** — 포트폴리오 프레임을 쓴 값이 여기 있다.
    · **이 시험은 '어느 쪽이 옳은가'가 아니라 '고치면 포트폴리오가 나아지는가'에 답한다 — 답은 아니다.**
      검증된 규칙은 닫힌 봉 쪽이고 형성 중인 봉은 검증된 적이 없다. 그런데 현행이 백테스트에서 나은 데에는
      **미검증 신호가 기여한다**(ih forming +5.28% vs closed −0.54%). 즉 **'현행 탐지가 옳다'는 증거가 아니라
      '지금 고치면 수익이 나빠진다'는 측정**이다. 둘은 다른 문장이고 어느 쪽을 택할지는 사용자 결정이다.
    · **사전 확률 '방향을 예측하지 않는다' 가 맞았다**(두 힘이 실제로 반대로 나옴). 예측 못 한 것 하나 —
      **비가산성**. 한계: current arm 은 부분봉 합성 **모델**이고 지연 0 가정이라 격차는 상한,
      holdout 절대 수준(전 arm CAGR −51~−76%)은 bear 지배 창이라 실거래 예보 아님.
- **포트폴리오 변동성 타겟팅 사전 등록 (2026-09-21, 사용자 지시 — ⑤)**: registry
  ...
  13 포지션이 사실상 하나의 베타 포지션인데 그 축은 시험된 적이 없다.
  · validate_portfolio 프레임 고정(배포 집합·방식D·슬롯 배분 current·MAX_POS·RISK_FRAC·LEV_CAP),
    **arm 이 바꾸는 것은 명목가 배율 하나뿐**. `vp.simulate(size_mult=)` 훅 신설 — **None 이면
    종전 동작과 완전히 같다**(test_port_vol §1 이 난수 6판으로 고정).
  · port_vol(주 판정): m = clip(TARGET/σ_p, 0.5, 2.0), σ_p 는 오픈북+신규의 연율 포트폴리오 변동성,
    ρ̄ 는 직전 60일 top30 **평균 쌍상관**(O(n) 역산, 전수 정의와 일치함을 테스트가 고정).
    **TARGET 은 train 의 σ_p 중앙값**을 holdout 에 그대로 적용. 진단 port_mkt/port_corr/port_dd.
  · 판정 **J1~J6 전부** — J1 CAGR / J2 Calmar / J3 부트 우위 ≥0.60 / J4 MDD 5%p / J5 train /
    **J6 노출 정합 0.90~1.10x**(그냥 작게 걸어 얻은 MDD 개선을 개선으로 세지 않는다).
  · **사전 확률: port_vol 이 current 와 거의 같을 것으로 본다** — ρ̄ 가 0.7~0.9 로 높고 변동이 작으면
    σ_p 는 개별 σ 와 거의 같이 움직이고 그건 건당 타겟팅이 이미 하는 일이다. 차이가 난다면 **ρ̄ 가
    튀는 급락 동조 구간**이고 그 표본은 적어 J3 를 못 넘길 가능성이 크다.
  · **실행 전 수정(공개)**: 진단 arm port_corr 의 초안 수식 m = sqrt(N_eff/N_ref) 는 **부호가
    반대였다**(N_eff 가 n 에 증가해 포지션이 늘수록 배율이 커진다). 테스트가 잡았고 결과 보기 전에
    상관 축 전용으로 고쳤다. **주 판정·판정 규칙은 한 글자도 안 바꿨다.**
  · **결과 REJECTED (run 35572055409) — J3·J5 탈락, 나머지 넷 통과. 실거래 무변경**:
    거래 8,817(train 4,188 / holdout 4,629) · ρ̄ 중앙 **0.639**(10~90분위 0.491~0.758) · TARGET = train σ_p 중앙 0.943.
    train Calmar current 0.88 vs port_vol 0.82(**J5 탈락**) / holdout **−0.81 vs −0.33**(J1·J2 통과) /
    **노출비 0.985x(J6 통과 — '작게 걸어 얻은 개선' 이 아니다)** / 부트 300회 우위 **50%**(**J3 탈락**).
    · **사전 확률 그대로다** — '차이가 난다면 ρ̄ 가 튀는 급락 동조 구간이고 표본이 적어 J3 를 못 넘길
      가능성이 크다'. holdout 이 크게 좋아졌는데 부트 우위가 정확히 50% 인 것이 '소수 구간이 끌어올렸다'의 서명.
    · **진단 arm port_mkt 은 train(1.33 vs 0.88)·holdout(−0.43 vs −0.81) 둘 다 current 를 이긴다** — 주 판정
      arm 이 못한 것을 했다. **그러나 진단 셀이고 사후 선택은 금지**(validate_ih_exit 교훈). 추격은 별도 사전 등록.
  · validate_port_vol.py / test_port_vol.py(70건) / port_vol.yml

- **업비트 슈팅 선행 신호 사전 등록 시험 (2026-09-21, 사용자 지시 "사전 등록해서 여러 창으로 돌려봐")**:
  registry `upbit_shoot_prereg_2026_09_21`. 같은 날 탐색 스터디가 최근 30일 **사건 11건**에서 본
  'D−1 에 거래대금·ATR·20일 고저폭이 크다'(베이스라인 중앙값 위 비율 0.82)를 **여러 해·여러 달 창**에서
  실행 가능한 규칙으로 다시 잰다. **DEPLOY_ON_PASS=False — 통과해도 실거래 반영 없음.**
  · **관측 단위를 사건에서 코인-일로 바꿨다** — 매일 종가에 한 특성으로 전 종목을 줄 세워 상위 10% 를
    사고 다음 날 종가에 판다(특성은 그 봉까지만, 인과). lift = P(슈팅|상위10%)/P(슈팅|전체).
    탐색 판은 사건 11건이라 표본 오차가 컸고 다중검정·창 분할·진입 규칙 형태가 없었다.
  · **귀무는 같은 날 같은 개수 무작위**(날짜별 초기하, 1000회, 시드 20260921). 슈팅이 특정 달에 몰려도
    그 편중이 귀무에 똑같이 들어가므로 '장이 좋았다'를 엣지로 오인하지 않는다. 통계량이 (n_d,e_d,k_d)
    에만 의존해 **한 귀무를 12 특성이 공유**한다 — 그래서 12 특성이 **같은 표본**을 써야 한다(요건: 이력
    180봉 이상 + 12 특성 전부 계산 가능 + 하루 유니버스 20종목 이상).
  · 창 = 달력 월, **2026-08·2026-09 는 판정에서 제외**(탐색 판이 본 창, 따로 병기만), 나머지를 시간순
    train 70% / holdout 30%. 가설 9 + **음성 대조 3**(dist_ma180·dd_250h·n_ma_above), Holm m=12.
    **대조 2개 이상이 C1 을 통과하면 판 전체 INVALID.**
  · 판정 C1 train lift ≥1.50 & Holm p<.05 & 월 일관성 ≥0.60 / C2 holdout lift ≥1.30 & p<.05 /
    **C3 실용성** 상위10% 평균 익일수익 − 전체 평균 >0 (두 분할) & holdout 상위10% 평균 − 왕복 0.1% >0.
  · **사전 확률(결과 전)**: lift ≥1.5 는 그럴듯하다(변동성 자기상관). **그러나 C3 에서 죽을 것으로 본다** —
    고변동 상위 10% 는 슈팅도 폭락도 더 자주 난다. xsec_chars 의 rvol20 이 정확히 그 구조('순위 예측이지
    평균수익이 아니다')였고 탐색 판에서도 슈팅 당일 추격은 D+1 중앙 −13.5% 였다. bb_width_pctile120 은
    A형(확장)·B형(수축)이 섞여 방향이 모호해 가장 먼저 탈락 예상 — **결과를 보고 빼면 사후 선택이라 뺄 수 없다.**
  · **근간은 특성 정의 동치** — 속도 때문에 새로 쓴 `upbit_feat.compute` 가 탐색 판의 `features()` 와
    **모든 키 비트 일치**여야 한다(test §1). 실제로 누적합 롤링에서는 드리프트 한 비트가 `n_ma_above` 를
    3↔4, 볼밴 백분위를 0.483↔0.475 로 뒤집어 **창합(참조와 같은 덧셈 순서)** 으로 되돌렸다.
  · 한계: 생존 편향(현 상장 종목만) · 단일 거래소 KRW(김치 프리미엄·상장 이벤트) · 스프레드 미반영 ·
    이력 180봉 요건이 신규 상장 구간을 제외 · 익일 종가 청산 하나만(D+5 는 진단).
  · **결과 (run 35669984048) — CONFIRMED 0 / 가설 9, 대조 0/3 통과(판 VALID). 실거래 무변경**:
    표본 32개월(2023-12~2026-07) · train 663일 75,339 코인-일 **사건 39**(기저 0.052%) / holdout 304일
    56,902 코인-일 **사건 17**(0.030%). 289 종목.
    · **죽인 것은 C3(실용성) 하나이고 12/12 전부다** — 상위 10% 의 익일 평균수익이 유니버스보다 낮다.
      train `거래량/20일평균` 상위 **−0.447%** vs 유니버스 **+0.024%** / holdout **−0.754%** vs −0.348%.
      진단 교집합(거래대금∩ATR)이 이 판 최고 lift(**train 2.57 / holdout 3.22**)인데 익일 **−0.26% / −0.64%**.
      **슈팅이 어디서 나는지는 맞히는데 그 바구니를 사면 잃는다** — xsec_chars 의 rvol20 이 남긴
      '순위 예측이지 평균수익이 아니다'와 같은 구조이고, **사전 확률이 정확히 이것이었다.**
    · **C1·C2 통과 셀이 분할마다 하나씩 다르다**: `vol_ratio20_prev5` train C1 통과(lift 2.73 Holm .012
      월일관 5/5)인데 holdout p .097 / `vol_ratio20` holdout C2 통과(lift 3.45 p .005)인데 train Holm .150.
      둘 다 '최근 거래량 급증' 한 가족이고 **어느 쪽도 C3 는 못 넘는다**(엣지 −0.37~−0.47%).
    · **제외 창(2026-08·09)이 실제로 예외였다** — lift 거래량 **7.35**(p .001) · 고저폭 4.90 · ATR 3.67 로
      판정 창보다 훨씬 높고, **그 창에서조차 엣지는 음수**(−0.32~−0.64%). **창을 늘린 값이 여기 있다** —
      한 창만 봤으면 lift 7.35·p .001 로 규칙을 만들었을 것이다.
    · **방법론 소득 — lift 만 보면 대조군도 2 를 넘는다**(1.49~2.23). 사건이 드물어 어떤 특성으로 줄 세워도
      상위 10% 가 사건의 20% 안팎을 잡는다. 날짜별 순열 귀무 + Holm 이 없었으면 **대조 특성조차 신호로 읽혔다.**
      사전 확률 '대조 lift ≈1' 은 그래서 부분 오답이고, 'bb_width 가 가장 먼저 탈락'도 오답
      (1.98 로 중간, 최약은 `turnover20_krw` 1.24 — 탐색 판에서 가장 크게 갈렸던 바로 그 특성).
    · 진단: A/B 유형이 분할마다 뒤집힌다(train A23/B16 → **holdout A3/B14**) — 탐색 판의 'A형 7/B형 4' 는
      일반적 구성이 아니다. 고가 라벨도 lift 안 커짐(1.06~2.48), D+5 도 음수, **월별 기저율 2024-03 0.228%
      → 2026-06 0.046% 로 슈팅 빈도 자체가 4배 줄었다.**
    · 한계 실현: 사건 56건뿐이라 C1 탈락 다수는 '표본 부족'일 수 있다(C3 는 표본 무관하게 부호 일관).
      이력 180봉 요건으로 같은 창에서 탐색 판 11건 → 8건(신규 상장 3건 제외).
    · **진입 규칙 후보 없음, 복귀 후보 없음.** 재시험은 사건 표본이 2~3년 더 쌓인 뒤. **C3 를 통과할 길이
      있다면 진입이 아니라 청산 쪽** — 고변동 바구니는 평균이 음수지만 오른쪽 꼬리가 슈팅이다(별도 사전 등록).
  upbit_feat.py / validate_upbit_shoot.py / test_upbit_shoot.py(54) / upbit_shoot.yml / report_upbit_shoot.md
- **B형(무거래 수축 뒤 급등) 전용 사전 등록 (2026-09-22, 사용자 지시 "B형만 공통점 더 찾아봐, A는 애초에 대상이 아니야")**:
  registry `upbit_bsqueeze_prereg_2026_09_22`. **DEPLOY_ON_PASS=False.**
  · **앞선 판은 B형을 찾을 수 없는 구조였다** — 12 특성을 전부 **내림차순**으로만 줄 세웠고
    `bb_width_pctile120` 내림차순은 '볼밴이 넓은 쪽' = A형이다. B형은 정확히 반대쪽이고 **규칙으로
    시험된 적이 없다.** 사용자의 'A는 대상이 아니다' 는 결과를 보고 고른 부분집합이 아니라 매매
    선호이고, 그것을 풀 정의로 옮긴 것이라 사후 선택이 아니다.
  · 바꾼 것 셋, 나머지(데이터·유니버스·창·귀무·시드·상위 10%·수수료)는 앞선 판과 **완전히 동일**:
    ① **A형 제외** — 직전 20일 수익 > +15% 코인-일을 풀에서 뺀다(문턱은 앞선 판에서 동결된 값)
    ② **수축 특성은 오름차순**(대조 3종은 비교 위해 내림차순 유지)
    ③ **라벨 지평 20봉** — E20 = 이후 20 거래일 안에 종가 +50% 가 한 번이라도. B형은 수축이 몇 주에
       걸쳐 풀리는 셋업이라 익일 라벨로는 원래 물음에 답할 수 없다. **창이 온전한 봉만**(절단 편향 방지).
  · **주 판정 셀 하나** — `squeeze_score` = 그날 풀 안 **볼밴 폭 120일 백분위 + 거래량/20일평균**의
    오름차순 순위 평균, 상위 10%(= 가장 조용하고 수축된 코인). 탐색 판 B형의 '250고점 −72%' 는
    **일부러 주 판정에서 뺐다**(사건 11건을 보고 얻은 값 → 넣으면 사후 선택), 진단으로만.
  · C1 lift ≥1.50 & Holm(**m=8**) p<.05 & 월일관 ≥0.60 / C2 holdout lift ≥1.30 & p<.05 /
    C3 상위10% fwd20 − 유니버스 > 0(두 분할) & holdout − 왕복 0.1% > 0.
  · **사전 확률(결과 전)**: **주 판정은 C1 에서 lift < 1 로 떨어질 것으로 본다** — 앞선 판에서 높은 쪽이
    2~3.4배 enriched 였으니 낮은 쪽은 depleted 일 가능성이 크고, 탐색 판 B형 4/11 은 lift≈1 과 구분 안 된다.
    **그러나 C3 는 처음으로 양수가 나올 수 있다** — 조용한 코인은 고변동 바구니처럼 매일 피를 흘리지 않는다.
    즉 흥미로운 결과는 **'잡지는 못하는데 잃지도 않는다'** 이고 그건 진입 규칙이 아니라
    **A형이 왜 손실인지에 대한 대조 증거**다.
  · 한계: 앞선 판 전부 + **E20 은 코인-일이 시간축으로 겹친다**(한 슈팅이 최대 20 코인-일을 양성으로)
    — 귀무는 같은 날 안에서만 섞어 영향 없으나 실질 독립 표본은 사건 수만큼뿐 · 수축 상위 10% 는
    회전율이 낮아 사실상 '조용한 코인 보유'에 가깝다.
  · **결과 (run 35672950835) — 주 판정 REJECTED, 대조 0/3(판 VALID). 실거래 무변경**:
    A형 제외 풀 120,804 코인-일 · train 618일 61,315 **사건 478**(기저 0.78%) / holdout 304일 53,178
    **311**(0.58%). **앞선 판의 '표본 부족' 유보가 사라졌다**(39/17 → 478/311) — 이번 기각은 검정력 문제가 아니다.
    · **주 판정 `squeeze_score` train lift 0.88** (478 중 44건, 기대 50) — **가장 조용하고 수축된 10% 는
      슈팅이 오히려 덜 난다.** 사전 확률 적중. 월 일관성은 전 특성 3~7/15 로 어디서도 절반을 못 넘고
      방향이 분할마다 뒤집힌다(squeeze 0.88→1.25, atr14 1.18→0.88).
    · **사전 확률의 나머지 절반도 적중 — 저변동은 실제로 덜 흘린다**: `atr14_pct` 낮은 10% 엣지
      **train +2.06%p / holdout +1.72%p**, `range20_pct` **+1.41 / +1.74** (두 분할 모두 양수).
      앞선 판 12셀 전부 엣지 음수의 기전이 '고변동'이었다는 **직접 대조 증거**이고 이 판의 실질 소득이다.
    · **그래도 C3 는 탈락** — holdout 유니버스 fwd20 가 **−6.59%** 라 저변동 상위 10% 도 절대값 −4.87%.
      **덜 잃을 뿐 버는 게 아니다.** holdout 이 하락 구간이라 절대 문턱이 사실상 통과 불가였다 —
      상대 엣지는 판정과 별개로 기록해 둘 값.
    · **예측 못 한 것: 두 축이 다르게 행동한다** — 변동성 축은 방어적인데 **거래량 축은 아니다**
      (`vol_ratio20` 낮은 쪽 절대 −0.71%). 합친 `squeeze_score` 가 각각보다 나쁘다(절대 −1.21%).
      '무거래 수축' 을 한 덩어리로 본 것이 설계의 약점.
    · 진단: 수축∩250고점낙폭(탐색 판 B형의 '−72%' 직격) train lift **0.68** / holdout **2.70** 로 완전히
      뒤집힘 — **주 판정에서 빼둔 판단이 옳았다**(넣었으면 holdout 만 보고 살렸을 것). 제외 창은 또
      lift 2.64 p .001 로 특별(엣지 −0.80%).
    · **두 판 합친 결론 — 포착과 방어가 정반대 방향이라 한 규칙으로 동시에 못 가진다.**
      고변동(A형) 포착 O(2.2~3.4) 방어 X(−0.45~−0.75%) / 저변동(B형) 포착 X(0.88~1.18) 방어 O(+1.4~+2.1%p).
      **B형은 실재하는 유형이지만 빈도로는 평균 이하다** — 사후에 그런 사례가 있다는 뜻이지 수축을 보고
      미리 고르면 확률이 올라간다는 뜻이 아니다.
    · **진입 규칙 후보 없음.** 남은 것은 사이징 방향의 방증 하나(변동성 타겟팅·rvol20 과 같은 방향의
      세 번째 독립 증거) — 업비트 현물 일봉 값이라 OKX 무기한에 그대로 옮기지 않는다.
  validate_upbit_bsqueeze.py / test_upbit_bsqueeze.py(40) / upbit_bsqueeze.yml / report_upbit_bsqueeze.md
- **알트 시즌 선행 지표 120일 지평 사전 등록 (2026-09-22, 사용자 지시 "120일로 사전등록해서 돌려봐")**:
  registry `altseason_prereg_2026_09_22`. **DEPLOY_ON_PASS=False.**
  · **계기**: 사용자 질문 "ETH/BTC·BTC.D·알트 시총 비율로 알트 시기를 판단할 지표가 충분히 검증된 게 맞냐" →
    아니다. 종전 라벨러 시험(regime_quality 2026-09-04, alt_side 2026-09-07)은 **전부 지평 20일 고정**이었고,
    alt_side 진단에서 지평을 늘리면 분리폭이 커지는 신호(40/60/90일 +3.4/+5.6/+8.4%p)가 있었으나 사후 선택
    금지로 멈춰 있었다. **120일은 사용자가 결과를 보기 전에 지정**한 값이라 이 구멍을 사후 선택 없이 연다.
  · 타깃 = `median_{알트}(120일 수익) − BTC 120일 수익`(알트 = BTC 제외·ETH 포함). 가설 10 + **음성 대조 2**
    (AR(1) φ=0.99 · 연 사인파 — 실제 지표와 같은 강한 자기상관, 경제적 내용 0). 둘 다 C1 통과 시 판 INVALID.
  · **이 판의 핵심은 중첩 처리다** — 120일 라벨은 이웃끼리 119/120 이 겹쳐 일별 상관을 그냥 재면 표본이 수십 배
    커 보인다. 귀무를 **회전 검정**(지표 계열만 ≥180일 원형 이동, 자기상관 보존·짝만 파괴)으로 만든다.
    합성 시험 실측 **회전 sd 0.431 vs iid 셔플 0.028 — 15배**. 중첩이 만드는 가짜 유의가 그만큼이었다는 뜻.
  · **MDE(최소검출효과) = 귀무 IC 95백분위를 판정에 넣었다** — |IC| 가 문턱(0.20)은 넘는데 MDE 보다 작으면
    REJECTED 가 아니라 **INCONCLUSIVE(검정력 부족)**. 2026-09-09 결정 ②의 연장이고, '검출 못 하는 층을
    판정에 두지 않는다'(L8 B 벤치 교훈)의 반대 방향 적용 — 층은 두되 **못 잡은 것을 기각으로 쓰지 않는다**.
  · 데이터 data_long 80종목 2017~2026-09-06, 네트워크 없음. 적격 180봉·그날 10종목 이상 → **실질 표본 시작
    2018-05-12**(실행 전 확인한 커버리지). train < 2022-01-01 ≤ holdout — 알트장이 양쪽에 하나씩(2021 / 2023-24).
  · **진짜 BTC.D·TOTAL3 는 이 판에 없다** — 레포의 BTC.D 는 CoinGecko 무료 5종 시총 프록시이고 **365일뿐**이라
    120일 지평에 비중첩 창 3개다. 진짜 지수는 유료. 도미넌스의 ①ETH/BTC 다리만 시험되고 ②시총 가중 다리는
    **측정 불가**로 남는다. 대신 가격으로 만들 수 있는 OTHERS/BTC 등가중 지수(PIT 편입)와 폭을 쓴다.
  · **사전 확률(결과 전)**: **CONFIRMED 0 예상.** 9년 창에 120일 비중첩 창이 train 약 11 / holdout 약 13,
    합쳐 **24개**뿐이라 MDE 가 0.30~0.60 에 잡힐 것으로 보고, 그러면 문턱을 넘는 셀도 대부분 INCONCLUSIVE 다.
    가장 큰 |IC| 는 btc_dist200·breadth180 예상(둘 다 '지금 상승장인가'의 대리 변수), btc_mom120 은 REVERSED
    가능성 병기, disp60 은 0 근처. **이 시험이 답할 수 있는 것은 '큰(|IC|≥0.2) 관계가 있고 확증 가능한가'
    뿐이고 '작은 관계가 없다'는 답할 수 없다.**
  · **통과가 0 이어도 소득 둘**: ① MDE 와 '몇 배 창이 더 필요한가' 숫자(이 물음에 답이 가능한지 자체를 정한다)
    ② **오늘 읽기 표**(지표별 현재 백분위·3분위, 판정과 무관한 진단).
  · 한계: 생존 편향(data_long = 오늘 OKX 에 있는 80종목 — 과거 알트 수익 과대) · 지표 10종 상호 상관 높음 ·
    '알트 초과수익' 하나로 알트 시즌을 정의(절대 상승과 구분은 진단).
  · **결과 REJECTED (run 35694986034) — CONFIRMED 0 / INCONCLUSIVE 0 / REVERSED 0 / REJECTED 10, 대조 0/2 → 판 VALID. 실거래 무변경**:
    report_altseason.md. 라벨 있는 날 2,917(train 1,327 / holdout 1,590), **비중첩 120일 창 24개**.
    · **기저가 결과의 절반이다** — 무조건부 120일 알트 초과수익 train **−4.8%** / holdout **−21.6%**.
      2022~2026 에는 **탈 알트 시즌이 애초에 없었다**. 12셀 전부 선호 3분위 절대값 −15~−33% 인 건
      지표가 나빠서가 아니라 바닥이 그만큼 깊어서다(생존 편향이 알트를 **과대**평가하는데도 이 수치).
    · **가장 정직한 숫자는 음성 대조** — 시드 고정 AR(1) 잡음의 |IC_train| **0.184** 가 가설 10 중
      **4개보다 크다**(ETH/BTC 기울기 .161 · 폭 변화 .004 · BTC 모멘텀 .065 · BTC 200MA .122).
      경제적 내용 0 인 계열이 자기상관만으로 그만큼 찍는다. **대조가 없었으면 그 정도를 '약한 신호'로 읽었을 것.**
    · **검정력** train **MDE 0.299** — |IC|=0.2 를 잡으려면 창이 **2.2배(≈8년, 4년 이상 더)** 필요.
      합성 실측 회전 귀무 sd 0.431 vs iid 셔플 0.028 — **중첩이 만들던 가짜 유의가 15배**였다.
    · **부호가 선언과 반대로 일관**(train 8/10 · holdout 9/10 음수)이나 **독립 증거 10개가 아니다** —
      ETH/BTC 3 · OTHERS/BTC+폭 3 · BTC 2 로 2~3 가족이고 어느 가족도 개별 유의하지 않다(양측 p 최소 .103).
      REVERSED 0 인 이유. 액면 해석('알트가 강해 보일수록 이후 더 나쁘다')은 **주장하지 않는다**.
    · **가장 가까운 셀 disp60(저분산)** — 선언 부호가 두 분할 다 맞고 선호 3분위가 기저를 **+9.3 / +6.5 %p**
      이기고 holdout p .025. 탈락은 **|IC_train| 0.194 < 0.20, 0.006 차** 하나. **쫓지 않는다** — ① 문턱을
      0.006 내리면 통과한다는 건 결과를 보고 안 것 ② **음성 대조도 두 분할 양수**(+8.5/+1.6%p) ③ **타이머가
      아니라 방어 신호**(저분산 3분위 holdout 절대값 여전히 −15.0% = '덜 진다') — 저변동 방어의 **네 번째
      독립 재현**(xsec_chars rvol20 · upbit_bsqueeze atr14 · 채택된 변동성 타겟팅). 기록만.
    · **홀드아웃이 막아준 것** — BTC 120일 수익률·BTC 200MA 위치가 **train 에서 선호 3분위 초과 +32.0 / +36.1 %p**
      ('BTC 가 먼저 뛰고 알트가 따라온다'가 2018~2021 엔 강하게 맞았다). holdout 에서 IC +0.12 → **−0.49**,
      초과 **−10.8 / −9.7 %p**. **train 최고 둘이 holdout 최대 반전.**
    · **오늘 읽기(2026-09-06, 전망 아님)** — ETH/BTC 3종 전부 상위 3분위(71/79/74%), 폭 0.600(69%)·**폭 20일 변화
      +0.500(97%, 10%→60%)**, OTHERS/BTC 중간(38/50%). 읽는다면 '**ETH 주도이고 넓은 알트로는 아직 안 퍼졌다**'.
      단 이 판은 그 어느 것도 120일 예측력이 있다고 확인하지 못했다 — 현재 상태이지 전망이 아니다.
    · **사전 확률 채점**: 적중 — CONFIRMED 0 · MDE 0.30~0.60(0.299) · '소득은 MDE 와 오늘 읽기 표'.
      오답 — '대부분 INCONCLUSIVE'(0, 부호가 반대로 가 크기 조건 전에 걸림) · '최대 |IC| 는 btc_dist200·breadth180'
      (실제 OTHERS/BTC 0.324/0.314) · **'disp60 은 0 근처'(실제 가장 가까운 셀)**.
    · **구조적 교훈** — '**어느 알트가** 오르나'는 코인-월 수천 관측의 횡단면 문제(xsec_chars 5,267)이고
      '**언제** 알트 시즌인가'는 **독립 관측 24개**의 시계열 문제다. 같은 9년인데 **자유도가 200배 차이난다**.
      이 물음만 레포에서 반복해 결론이 안 나는 이유는 지표 선택이 아니라 이 구조이고, **지평을 늘릴수록 더 나빠진다**.
    · **결함 공개** — `tercile_stats` 가 선언 부호를 안 보고 늘 '높은 쪽' 3분위를 잡아 음의 부호 셀(disp60)에서
      **가설이 나쁘다고 본 쪽을 C3 에 넣고 있었다.** 1차(35694689397) 뒤 수정·재실행. **판정은 바뀔 수 없었다** —
      disp60 은 C1 에서 독립 탈락, 나머지 9 는 sign>0 이라 동작 동일(두 실행 판정·IC·p 동일, disp60 3분위 진단
      −28.9%→−15.0% 만 변경). 같이 고침 — '오늘 읽기'가 선언 부호로만 찍어 거꾸로 읽힐 수 있었다.
    · **미측정으로 남은 것**: 진짜 BTC.D/TOTAL2/TOTAL3(무료 API 365일 한계) · 작은 관계(|IC| 0.1~0.2, MDE 아래 사각지대).
      **복귀 후보 없음. 재시험은 알트장이 한 번 더 지나 비중첩 창이 30개를 넘을 때.**
  validate_altseason.py / test_altseason.py(49) / altseason.yml / report_altseason.md
  · **검정력 곡선 — 유료 데이터 결제 판단 (2026-09-22 사용자 지시 "검정력 곡선 돌려줘", run 35697247459)**:
    registry `altseason_power_curve_2026_09_22`. **판정 아님, 계산 전용.** 유료 데이터를 사면 타깃을
    진짜 TOTAL3/BTC 로 바꿔 표본 시작이 2018-05 → 2014, 비중첩 창 24 → 약 38 이 된다. 그만큼으로
    MDE 가 문턱 0.20 밑으로 오는가. **검산: train MDE 0.299 를 정확히 재현.**
    · **적합 MDE = 0.777·n^(−0.378), R² 0.888.** **p=0.378 < 0.5** — 120일 창 하나가 독립 관측 하나가
      아니다(국면이 뭉쳐 비중첩 창끼리도 상관). **정보가 독립 표본보다 느리게 쌓인다**, MDE 반감에 창 6.3배.
    · **결론: 지금 사지 않는다.** 2014 시작 시 **train(창 21) MDE 0.246 — 문턱 미달**. 전체(창 38) 0.197 은
      문턱과 1.5% 차라 적합 오차(전·후반 0.228 vs 0.283) 안. 필요량은 **창 36개 = 11.9년**이고 train 만으로
      채우려면 전체 20년 이상 — 암호화폐에 없다. **내 앞선 '1/√n 으로 0.22, 거의 닿는다'는 p=0.5 가정
      탓이었고 철회한다.**
    · **가장 값어치 있는 발견 — 병목은 예산이 아니라 지표의 지속성이다.** 전체 표본 지표별 MDE
      breadth_chg20 **0.136** / ethbtc_slope20 0.183 / … / altbtc_dist200 **0.326**. **최저/최고 차이가
      데이터 10.2배에 해당**한다. 10배는 못 사지만(80년치) 지표 선택은 공짜고, 최저 둘이 정확히
      **차분형**(폭 20일 변화·ETH/BTC 기울기), 최고 둘이 가장 지속적인 수준형이다.
      **단 공짜 점심 아님** — 차분은 MDE 와 함께 진짜 IC 도 지울 수 있다(다른 베팅이지 우월한 베팅 아님).
    · **권고**: ① 유료 결제 보류 ② 같은 아이디어의 **차분형·저지속성 판** 사전 등록(0원) ③ 그래도 경계값이
      남으면 CoinGecko Analyst 1개월($129) + 고정 체크리스트. **트레이딩뷰는 해당 없음**(공식 API 없음,
      CSV 는 화면에 뜬 것만 수동 — 자동 파이프라인 불가).
    power_curve.py / test_power_curve.py(33) / power_curve.yml / report_altseason.md 부록 A
  · **저지속성(차분·백분위) 판 — 공짜 레버를 쓰고 답을 얻었다 (2026-09-22 사용자 지시 "차분형·저지속성
    지표 사전 등록을 지금 설계", run 35700854897)**: registry `altseason_fast_prereg_2026_09_22`.
    **CONFIRMED 0 / INCONCLUSIVE 1 / REJECTED 9 · 대조 0/3 → 판 VALID.** 타깃·분할·귀무·judge 를
    level 판에서 그대로 import(테스트가 11함수 `is` 동일성 고정), 바꾼 건 지표 구성뿐.
    · **검정력 곡선의 약속은 실현됐고 예측보다 좋았다** — accel MDE **0.093~0.119** vs level 0.231~0.403,
      **2.0~4.3배 감소 = 데이터 6~25배**(유료 데이터가 사는 1.6배와 비교 불가).
    · **그런데 그 해상도에서 아무것도 없다 — 이제 검정력 문제가 아니다.** accel 5셀 IC
      +0.041~+0.116, 전부 자기 MDE 아래·문턱 0.20 의 절반 이하. **볼 수 있는 해상도로 봤고 없었다.**
    · **pctile 구성은 실패** — 5 중 3에서 MDE 가 올랐다(분산 0.339→0.463). ac120 이 기전 —
      pctile 자기상관 0.378/0.324 가 level 기준(dist200·breadth180) 0.146/0.171 보다 **높다**.
    · **두 판 30셀 합친 것이 진짜 발견** — rho(MDE,|IC|) **+0.490**, **|IC|/MDE 비 중앙 0.57**
      (순수 잡음 기대 ≈0.6), **잡음 대조 5개의 비 0.33~0.56 이 실지표 분포 한가운데**.
      **level 판에서 가장 커 보였던 IC(0.324·0.314)는 그 셀이 가장 시끄러웠기 때문이다.**
      비>1 인 다섯 셀은 **전부 선언 부호와 반대** — 잡음의 서명. (rho +0.49 는 중간이고 산포가
      넓어 '모든 IC 가 잡음' 까지는 주장 안 함.)
    · **유일 비기각 disp_pctile** — IC −0.323 부호 일치·상대 엣지 +6.50%p 인데 **MDE 0.463 으로
      30셀 중 최대**라 INCONCLUSIVE. 저변동 방어의 **다섯 번째 독립 재현**이나 비 0.70 으로 잡음
      범위 안이고 절대값 −15.1%('덜 진다'). **추격 안 함.**
    · **사전 확률 — 절반 틀렸다.** 적중 'accel IC≈0'·CONFIRMED 0 / 오답 'pctile 이 진짜 후보'·
      'pctile MDE 0.18~0.25'·'막을 층은 C3'(실제 C1 에서 먼저 걸림). **그리고 사전 등록의 유보를
      스스로 반증** — breadth_chg20 의 0.004 를 '잡음 draw 일 수 있다'고 적었으나 roc 5셀
      0.004~0.136·accel 5셀 0.041~0.116 으로 일관, **대표값이었다.**
    · **닫힌 것 / 열린 것**: 빠른(차분) 축은 닫혔다(좋은 해상도로 보고 없음). 느린(수준) 축은
      여전히 MDE 0.23~0.40 이라 미해결이나 30셀 분석이 잡음과 구분 안 됨을 시사.
      **유료 데이터 권고 불변 — 사지 않는다.**
    validate_altseason_fast.py / test_altseason_fast.py(60) / altseason_fast.yml / report_altseason.md 부록 B
- **외부 증거 기록 — BitMEX 'aoa' 무손절 사다리 분석 (2026-09-22, 사용자 제공 핸드오프 + 지시 "적용할 항목 있는지 검토")**:
  registry `ext_evidence_bitmex_aoa_2026_09_22`. **시험이 아니다 — 레포 밖 데이터의 관찰을 기존 결론과 대조해 기록만
  한다. 실거래·코드 변경 없음, research_log 에도 넣지 않는다(실행 기록이 아님).**
  · **매매 규칙으로 적용할 항목 없음 — 독립된 이유 셋**: ① **불변 안전장치 위반** — 무손절 마틴게일(2021년 손절 0건,
    물타기 57~61%)은 '손절 주문 없으면 실거래 절대 안 됨'이 금지하는 그 형태다. '계좌 킬스위치로 대체' 제안은 대체가
    못 된다(EQUITY_FLOOR 는 이미 있고, 계좌가 녹은 뒤 켜진다). ② **게이트 통과 불가 + 사후 선택** — 규칙화 격자가
    1h 96셀 중 66% 손실·중앙 Sharpe −0.30 / 5m 48셀 중 50% 손실·중앙 0.01 인데, Sharpe 0.65~1.17 셀을 고르는 것은
    tb_wide·cadence 가 금지한 격자 최대값 사후 선택이다. 전부 in-sample. **손절을 붙이면 음수로 돌아선다** = '손절
    없음'이 엣지의 조건 → ①과 화해 불가. ③ **마찰 미반영**(핸드오프 자신이 명시) — 이 레포는 펀딩을 건당 0.073%p
    = 엣지의 4.3% 로 쟀고, cadence 에서 슬리피지 0.141% 하나로 weekly Calmar 0.56→0.19 가 무너졌다. 사다리는 체결
    횟수가 많아 마찰 민감도가 레포의 어떤 규칙보다 높다.
  · **기록하는 값 ① 가격 피처로 방향이 안 잡힌다는 독립 재현** — 외부: 16 가격 피처 + 시계열 CV 방향 정확도 **50~52%**,
    진입 피처가 승패도 예측 못 함. 레포: regime_quality 방향 적중 **49%** · xsec_chars 24 변수 중 1개(그나마 순위 예측이지
    평균수익 아님) · altseason 30셀 |IC|/MDE 중앙 0.57 ≈ 잡음 기대 0.6. **다른 거래소·다른 기간·다른 방법에서 같은 답**이면
    프레임 탓이 아니라 성질이다.
  · **② 사다리 = 앵커 리셋, 이미 분해돼 있다** — cadence_prereg: none 0.34 → **anchor 1.09(+0.75)** → reweight 1.21(+0.11)
    → reselect 1.01(**−0.20**). 매 1h 종가 EMA50 재앵커가 같은 기전이고, 마찰 물리면 안 남는 것도 같이 측정됐다. 재실행 가치 없음.
  · **③ 핸드오프 처방(비가격 입력 4축)의 레포 매핑** — 펀딩 극단 **기각 완료**(≤1h 5축) · 청산 캐스케이드 **가격·거래량
    대리지표로 배포**(진짜 청산 피드 아님) · OI **수집만**(perp_daily, 종목별 이력 없어 시험 불가) · **호가 불균형 = 미시험,
    데이터 없음**(틱/호가 필요, 레포는 봉뿐 — 사전 등록 불가) · **거래소 간 프리미엄 = 미시험, 데이터는 만들 수 있다**
    (업비트 KRW REST 가 이 환경에서 열린다) → 김치 프리미엄 축 사전 등록 대상.
  · 부수 대조 둘: '1% 고정 익절·승률 거의 100%·그래도 손실'은 익절 여섯 번째 기각(tp_1h 의 '승률 88%·건당 −0.28%')의
    외부 사례 / '상위 10일이 PnL 의 51~96%'는 게이트 top5 기여도가 잡으려는 모양이고 10/06 이후 '승률 → 복권형 지표
    대체' 설계의 근거를 보탠다.
- **거래소 간 프리미엄(김치 프리미엄) 사전 등록 (2026-09-22, 사용자 지시 "호가 불균형/김치 프리미엄 축을 사전 등록으로 설계")**:
  registry `kimchi_prereg_2026_09_22`. 위 외부 증거가 꼽은 비가격 입력 4축 중 **레포가 데이터를 만들 수 있는 유일한 축**.
  **호가 불균형은 설계 불가로 남긴다** — 틱/호가 이력이 필요한데 레포는 봉 데이터뿐이고(band_rule·tp_1h 에서 반복해 걸린
  구멍), 지금 등록하면 잴 수 없는 것을 등록하는 것이 된다. **DEPLOY_ON_PASS=False — 통과해도 실거래 반영 없음.**
  · **두 가족, 각자 부모 프레임을 통째로 상속(분할까지)** — 섞어서 새 분할·문턱을 만들면 그게 사후 선택이다.
    **가족 X(주 판정)** 횡단면 종목 선택, 부모 = validate_xsec_chars(월말 형성·PIT liquid·fwd20·월 블록 부트·Holm·
    train<2025-01·MIN_CS 15·왕복 0.4%), 셀 4 = kp_rel / chg5 / chg20 / z60 ·
    **가족 T(보조)** 시계열 방향, 부모 = validate_altseason(회전 귀무·MDE·judge·IC1 .20/IC2 .15·train<2022-01-01),
    지평만 120→**20**(MIN_SHIFT 30), 셀 3 = kp_mkt / chg20 / z120, 타깃 = 적격 코인 중앙 fwd20.
    **문턱은 낮추지 않았다** — 지평이 짧아 IC 가 작아질 여지가 있지만 낮추면 사후 완화다. test §1 이 13함수 `is` 동일성 고정.
  · **환율이 정확히 소거되는 것이 주 판정의 근간** — `kp_rel_i = ln(KRW_i/USDT_i) − 그날 전 종목 중앙값` 이고 환율은
    같은 날 모든 코인에 공통 곱이다(test §2 실측 최대차 1.8e-15). 그래서 가족 X 는 환율 데이터가 필요 없다.
    가족 T 는 시장 전체 수준이라 ECB 고시를 쓰고 **오염이 남는다**(14:15 CET 고시 vs 00:00 UTC, 평일만).
    정통 김프(업비트 KRW-USDT 환산)는 **그 시장이 2025-06-15 상장**이라 train 이 없다(실행 전 확인).
  · **봉 정렬이 데이터의 전부** — 업비트 일봉은 KST 기준이라 UTC 일봉과 **9시간** 어긋나고, 알트 일변동 5% 면 그 표류가
    약 3% 로 재려는 상대 김프 스프레드(1~3%)보다 크다. 업비트 **240m 은 UTC 정렬**이라 `D 20:00` 봉 종가 =
    D+1 00:00 UTC = data_long date=D 종가 시각 → 240m 을 받아 20:00 봉만 남긴다(build_data_kimchi, test §3).
  · **사전 부호는 4+3셀 전부 '-'** — 레포가 세 번 독립 재현한 '최근 거래대금·관심 증가 → 이후 나쁨'(xsec_chars
    vol_ratio_30_90 REVERSED · episode_profile REVERSED · breadth_state) + 김프 평균회귀. 셀마다 부호를 다르게 두면
    양쪽으로 이기는 공짜 승리가 된다.
  · **설계 부채 소진 — 가족 X 는 train 에도 TOP−유니버스 > 0 을 요구한다.** xsec_chars 가 '순위 통과·평균 실패 셀이
    CONFIRMED 로 찍힌다 → 다음 프레임은 train 에도 요구(사후 변경 안 함)'로 남긴 구멍이고 여기가 그 다음 프레임이다.
    test §6 이 '부모 finalize 로는 통과하는 셀이 이 요건에서 REJECTED 가 된다'를 실증한다.
  · 음성 대조 X 3(코인별 AR(1) 둘 + **코인별 상수** = 정보 0·지속성 최대) / T 2. **대조는 Holm 미보정 + 방향 자유**
    (가장 통과하기 쉬운 조건 = 가드로는 보수적), 한 가족에서 2개 이상 통과 시 그 가족 INVALID.
  · 데이터 data_kimchi/ — 업비트 KRW ∩ data_long = **62 종목**, 2017~2026-09-21. 커밋해서 러너는 네트워크 없이 읽는다.
  · **사전 확률(결과 전)**: CONFIRMED 0 예상 · 가족 X 안에서는 **kp_rel(수준)이 가장 약하고 z60·chg 가 나을 것**
    (수준은 상장·유동성 코인 고정효과가 지배 = 검정력 곡선이 짚은 지속성 함정) · 가장 있을 법한 흥미로운 결과는
    **REVERSED**(한국 자금이 모멘텀)이고 그래도 승격 안 함 · **가족 T 는 지평 20 이라 MDE 가 알트시즌 120일 판보다
    훨씬 나을 것 — 거기서 뭔가 나오면 그게 이 판의 실질 소득**(단 홀드아웃 2022~2026 은 bear 지배). 크기는 예측 안 함.
  · 한계: 이중 생존 편향 · data_long 이 종목마다 다른 현물 거래소라 그 가격차가 비율에 섞인다(중앙값 차감은 공통분만
    지운다) · 원화 입출금 제한·전송 시간 미반영이라 '프리미엄이 높다'가 곧 차익 기회가 아니다(예측력만 본다).
  · **결과 (run 35744717876) — 가족 X CONFIRMED 0 / REJECTED 4(REVERSED 1) · 가족 T CONFIRMED 0 / REJECTED 3 ·
    대조 X 0/3 · T 0/2 → 두 가족 다 판 VALID. 실거래 무변경**: report_kimchi.md. 격자 2019-05-03~2026-09-21
    (2,695일) · 62 종목 · 코인-월 2,922 · 88개월(train 67 / OOS 20).
    · **가족 X — 네 셀 전부 선언 부호와 반대**(정렬 IC −0.018~−0.057). 원 관계는 '상대 김프 높은 코인이 이후 더 오른다'이고
      kp_rel_z60 이 양측 p<.05 로 **REVERSED**. **단 train 한정** — OOS 는 네 셀 모두 p .57~.85 로 무작위와 구분 안 됨.
    · **REVERSED 를 쫓지 않는 이유 — D5 가 정체를 말한다**: kp_rel 의 xsec 상관 상위가 range_pos_20 −0.20 · rsi14 −0.18 ·
      ret_1w −0.17 로 **상대 김프가 높은 코인 = 최근 많이 빠진 코인**이다. 원화 호가가 하락을 늦게 따라가면 기계적으로
      프리미엄이 생긴다 → REVERSED 는 새 정보가 아니라 **레포가 이미 아는 단기 반전의 재포장**일 수 있다.
    · **가족 T — kp_mkt 은 선언 방향이 맞았다**(IC −0.189: 시장 프리미엄이 높을수록 이후 20일 중앙 수익이 낮다).
      그러나 문턱 0.20 **바로 아래**이고 **자기 MDE 0.264 아래**라 이 판이 검출할 수 없는 크기이며 OOS −0.018 로 재현 안 됨.
    · **연 주기 사인파 대조가 가족 T 최대 |IC| 0.216 — 실제 지표 어느 것보다 크다**(kp_mkt 0.189). 부호가 반대라 탈락했을 뿐,
      경제적 내용 0 인 계열이 자기상관만으로 그 정도를 찍는다. **알트시즌 판(잡음 0.184)에 이은 두 번째 실증** —
      대조가 없었으면 −0.189 를 '문턱 근접 신호'로 읽었을 것. **지속성→MDE 기전도 독립 재현**(ac20 최저 chg20 의 MDE 0.179 최소).
    · **진단 셀 하나가 모든 층을 통과했다 — kp_volshare**(한국 거래 집중도 = ln(업비트 KRW 점유율/USDT 점유율)):
      dir '−' · train IC **+0.080** ICIR 2.87 양측 p **.002** 연도양수 67% · OOS IC **+0.066** p **.046** ooTOP **+0.85% > 왕복 0.4%**.
      읽으면 **한국 거래 집중도가 낮은 코인이 이후 더 오른다**. **승격하지 않는다** — ① 진단으로 분류한 이유가 측정 문제
      (업비트 4h 슬라이스 vs USDT 1d)이고 **결과 전에 적어 둔 것** ② 방향이 '?'라 사전에 부호를 건 셀보다 자유도가 하나 많다
      ③ 사후에 진단에서 골라 살았다고 하지 않는다. **후속(사용자 결정)**: 슬라이스를 맞추고 부호를 사전에 걸어 별도 사전 등록 —
      방향이 맞다면 '최근 거래대금·관심 증가 → 이후 나쁨'의 **다섯 번째 독립 재현**이고 재료가 국내 거래소라 축이 새롭다.
    · **D2 가 배관을 확인해 준다** — 연도별 중앙 프리미엄 2019 +0.30% / **2021 +3.32%** / 2024 +2.10% / 2026 −0.05%,
      최근 −1.42%(역프). 알려진 김프 역사와 맞는다.
    · **사전 확률**: 적중 — CONFIRMED 0 · 'REVERSED 가 가장 있을 법하다'(정확히 z60) · 크기 예측 안 함. **절반 오답** —
      '수준이 가장 약하고 z60·chg 가 나을 것'은 |IC| 순서로는 z60 최대가 맞지만 **선언과 반대 방향**이고 최약은 chg5.
      미실현 — '가족 T 에서 뭔가 나오면 실질 소득'은 안 나왔고 대신 방법론 소득이 나왔다.
    · **닫힌 것**: 김프는 이 프레임에서 종목 선택에도 방향에도 못 쓴다(주 판정 7셀 전부 기각, OOS 재현 0).
      비가격 입력 4축 중 **셋째가 닫혔다**(펀딩·캐스케이드에 이어). 남은 것은 호가 불균형(데이터 없음) 하나.
    · **공개**: 커밋 전 부분 데이터(27종목)로 배관 스모크 1회 — 종료 코드와 예외만 확인, **수치는 보지 않았고** 산출물은 지웠다.
  build_data_kimchi.py / validate_kimchi.py / test_kimchi.py(55) / kimchi.yml / data_kimchi/

## 다음 할 일
- [x] **김치 프리미엄 사전 등록 결과 판독 완료 (2026-09-22, run 35744717876)** — 가족 X·T 주 판정 7셀 전부
      REJECTED, OOS 재현 0, 대조 0/5 → 판 VALID. 실거래 무변경
- [ ] **kp_volshare(한국 거래 집중도) 별도 사전 등록 (사용자 결정)** — 김치 판의 **진단 셀**이 train(IC +0.080
      p .002)·OOS(+0.066 p .046)·비용(ooTOP +0.85% > 0.4%) 전 층을 통과했다. 진단인 이유는 **슬라이스 불일치**
      (업비트 4h vs USDT 1d)이고 결과 전에 적어 둔 것이라, 강등이 아니라 **제대로 재야 하는 후보**다.
      해야 할 것: 업비트 240m 6봉 합산으로 슬라이스를 맞추고 **부호를 사전에 걸어** 다시. 방향이 맞다면
      '최근 거래대금·관심 증가 → 이후 나쁨'의 **다섯 번째 독립 재현**이고 재료가 국내 거래소라 축이 새롭다
- [ ] **호가 불균형 축은 데이터 확보부터** — 외부 핸드오프가 꼽은 비가격 입력 4축 중 유일하게 **사전 등록 자체가
      불가**한 축이다(틱/호가 이력 없음, 레포는 봉 데이터뿐). 수집 경로 설계가 선결 과제
- [x] **②③⑤ 결과 판독 완료 (2026-09-21)** — ② MATERIAL(약한 조건 하나) / ③ MATERIAL(가장 큼) /
      ⑤ REJECTED. 셋 다 실거래 무변경
- [ ] **③ 후속 — `detect_on_closed_bar` 를 기존 배포 패턴에 켤 것인가 (사용자 결정, 관찰 종료 후)**:
      ...
      · **측정 완료 — 답은 '켜면 나빠진다' (2026-09-21, run 35577424571)**: 주 판정 closed_1d **REJECTED**
        (holdout Calmar −0.81 vs current −0.56, train 0.88 vs 2.04, 부트 우위 33%). 스위치
        (`universe.json["closed_bar_patterns"]`, 기본 빈 목록 = 종전 동작)는 넣어뒀으니 10/06 에 켜는 것은
        **한 줄**이지만, 수치는 켜지 말라는 쪽이다. **단 이것은 '현행이 옳다'가 아니라 '고치면 수익이
        나빠진다'** — 현행 우위에 미검증 신호(ih forming +5.28% vs closed −0.54%)가 기여한다.
        검증-실행 일치를 택할지 수익을 택할지가 사용자 결정으로 남는다. **지금 실거래는 무변경**
...
- [ ] **validate_portfolio prio_edge 채택 여부 (사용자 결정)** — 레짐 청산을 켜고 재실행하니
      holdout Calmar current −0.81 vs prio_edge −0.57 로 **9/10 판정이 뒤집혔다**. 단 REGMAP 수정 +
      MAX_POS 16→13 + RISK 1.0→1.5% 가 동시에 바뀐 교란이라 원인 귀속 불가 — 가르려면 한 번에 하나씩
- [x] **sizing_vol 재실행 — 판정 유지 확인 (2026-09-21)** — 전 표본·라우팅 둘 다 4/4 통과.
      변동성 타겟팅 채택 근거는 레짐 청산 결함과 무관했다
...
- [ ] **equity $533 하한 주시** (2026-09-10 갱신 — risk 1% 하향으로 $355 → $533) — 전 신호 통과
      ...
      · **2026-09-16 00:05Z equity $517.13 — 문턱을 처음 넘어섰다(여유 −$16).** 24h 동안 $582.66 → $517.13
        (**−$65.5 / −11.2%**): 손절 4건(INJ fvg −8.24% · XRP marubozu −8.15% −$6.25 · GAS three_soldiers_4h
        −8.23% −$6.54 · CRV equal_lows_4h −7.46% −$4.41) + 만기 2건(SOL +0.74% · ETH triple_bottom_4h −0.27%)
        + 오픈 평가손. **자금 이동 아님** — free 가 진입·청산을 따라 317.45 → 240.92 → 279.42 → 243.89 →
        258.85 → 221.15 로 움직였고 equity 와 1:1 이 아니다.
        · **지금 뜻**: equity $517.13 이 지원하는 최소 배율은 **0.4641** → **σ 약 155%/yr 초과 신호부터 주문이
          안 나간다**(배율 하한 0.4507 = σ≥160% 는 $532.50 필요). 9/15 실제 진입 4건은 σ 41~109%(배율
          0.66~1.77)로 전부 여유 안이었고 **`필요 equity` 스킵 로그는 아직 0건**.
        · 낙폭 자체는 이례가 아니다 — risk 1% 격자의 boot MDD 중앙이 −45.4%(p10 −65.1%)라 −11% 는 그 안.
        · registry `equity_threshold_crossed_2026_09_16`. **충전·위험수준 조정은 사용자 결정.**
...
- [ ] **supabase_schema_patch_2026_09.sql 실행** (사용자, SQL Editor) — 실행 전까지 entry_ts/tf/
      entry_regime 등이 복원 시 유실. **2026-09-17 에 qty/leverage/sl_order_id 3컬럼 추가** —
      이게 없어 tp1 시장가 청산이 메인 레그까지 닫았다(ONDO). 코드는 지분 비율 폴백으로 안전하게
      돌지만, 실행하면 계약 수를 정확히 알고 'OCO 이미 집행' 가드도 살아난다
...
- [ ] **tp1_engulfing_1h $70(9/10 상향 전 $30) 강제 실행 대조 보고** — 진입 30건 또는 10-06: 실거래 승률·건당·수수료 합·포트 잔액 vs tp_1h(88.00% / −0.28%). 정지·충전은 사용자 결정
      ...
      · **4건차 (ETH 롱, 2026-09-14 21:03Z 청산) — 익절.** 진입 2542.14(9/12 14:03Z) → 체결 **2566.2475**
        (익절선 2567.56 대비 −0.052%p 미끄러짐) 가격수익 **+0.9483%** → 수수료 0.2% 차감 ×3x = **+2.2449%**,
        포트 74.91 → **약 $76.59**. free +$76.30.
        · **해소 경로가 예상과 달랐다** — 겹친 종목이라 `settle_by_algo` 로 잡힐 줄 알았으나 실제로는
          **엔진 자체 D청산**(`[live] ETH long D청산 실행 (atr_target)`)이었다. 청산 시점에 ETH 실포지션이
          tp1 단독이었고 eval_I 가 배리어 도달을 봉 데이터로 보고 시장가로 닫았다. **max_open 은 정상 해제**
          (직후 슬롯점유에서 tp1 사라짐, 이후 캡 스킵 없음).
        · **해소 — 장부 기준으로 확정(2026-09-16)**: 5건차 진입 로그가 `cap_margin=$76.70`(= 74.91×1.024)이다.
          체결 기준 $76.59 가 아니다. **즉 엔진 원장은 청산 로그가 `fill=None` 이면 체결가가 아니라 봉 기준
          (배리어 정확히 +1%)으로 기록한다.** 차이 $0.11 는 낙관 편향으로 포트에 누적되므로, 대조 보고 때
          장부 포트와 OKX 실현 손익을 나란히 적어 드러낸다. 코드 변경 아님(관찰 기간).
        · **보유 55시간**(9/12 14:03Z → 9/14 21:03Z). tp_1h 검증 보유 중앙 17봉의 3배 — 이상은 아니나
          1포지션 캡에서는 이게 곧 기회비용이다(아래).
      · **5건차 (AAVE 롱, 2026-09-15 13:03Z 진입, 미결)**: `cap_margin=$76.70` 3x 명목 $230.10.
        신호 128.00 → 체결 **128.7922 = 진입 슬리피지 +0.62%**, 이 판에서 가장 큰 미끄러짐이다(1~4 건은
        0.02~0.07%p). **체결가 기준 배리어 재정렬은 정상 작동** — stop 118.4889 / target 130.0802 로 OCO
        재등록. AAVE 는 메인과 안 겹쳐 선택지 B 의 생략 조건에 해당하지 않는다. 같이 찍힌
        `[SL점검] AAVE 손절 누락 → 재등록 @ 118.49` 는 재정렬이 기존 algo 를 취소하고 다시 거는 설계
        경로이지 이상이 아니다. **진입가가 밀려도 ±1%/−8% 기하는 보존된다**(백테스트는 신호봉 종가 진입
        가정이라 절대 수익만 달라진다).
      · **누적 4건 4승 + 1건 미결** (tp_1h 검증 승률 88.00% 대비 표본 부족 — 판단 유보).
        포트 70 → 71.63 → 73.19 → 74.90 → **76.70**(장부 기준). **손절 시 $70 리셋 경로는 아직 미검증** —
        5건차가 손절이면 그게 첫 검증이다.
      · **30건 목표는 사실상 도달 불가 — 10/06 이 실질 종료 시점**: 9/09 시작 후 5일에 4건이고, 4건차가
        혼자 55시간 동안 슬롯을 잡는 사이 **캡 스킵 10건**이 쌓였고, 5건차도 같은 자리를 막아 9/15 에만
        3건 더 늘었다(AVAX·TAO·BICO → **누적 13건**). 남은 기간을 다 써도 15~20건. 1포지션 캡(사용자 지정 `live_cap.max_open=1`)의 구조적 결과다.
      · **해소 (2026-09-17) — (나) 확정, OKX 이력 확인 불필요.** 9/16 ONDO 가 같은 구조를 더 깨끗한
        숫자로 재현했다(진입 시점 장부 11 = 실포지션 11 로 유령 0 이었는데 tp1 D청산 뒤 실포지션만
        −3). 근본 원인은 `live_order` 가 DB 왕복에서 유실돼 close_qty_for 가 전량으로 떨어진 것이고,
        같은 수정으로 두 건 다 막힌다 — 위 '겹친 종목 시장가 청산' 항목. 종전 기록:
      · **미해결 관찰 — 22:03Z ETH triple_bottom_4h 행 reconcile-close(사용자 확인 필요)**: `fill=2566.2475`
        (= tp1 의 체결가) `−0.07%` 실현 **+$1.60**. 로그만으로 두 해석이 안 갈린다.
        · **(가)** triple_bottom ETH 포지션은 그 전에 이미 닫혀 장부에만 남아 있었고, reconcile 이
          (symbol,direction) 로 매칭하면서 **tp1 의 체결가를 남의 행에 붙였다** → −0.07% 는 가짜 수치
          (tp1 청산가 ÷ triple_bottom 진입가). **실포지션 수가 21:03Z 에 10→9 로 떨어지고 22:03Z 엔 9 로
          불변**이라 이쪽이 유력하다.
        · **(나)** tp1 의 시장가 청산이 triple_bottom 레그까지 끌고 나갔다 = `close_qty_for` 클램프 미작동.
          이 경우 **배포된 triple_bottom_4h(v5 CONFIRMED 셀)의 청산이 검증 규칙과 달라진 것**이라 더 무겁다.
        · 가르는 방법은 **OKX 포지션 이력에서 ETH 청산 건수·시각을 보는 것** — 이 환경에서는 조회 불가.
          겹친 종목 문제의 세 번째 얼굴이고, 선택지 B 는 OCO 경로만 고쳤다(엔진 시장가 청산 경로는 별개).
...
- [ ] **승률 문턱 → 복권형 지표 대체 사전 등록 (2026-10-06 관찰 종료 후, 사용자 결정 2026-09-15)** —
      `gate.dist_ok` 의 승률 조건을 top5 기여도·절사평균갭으로 교체. 근거 A1(rho 승률-절사갭 +0.786 / -top5 -0.573 /
      -Calmar -0.183). 두 지표는 이미 게이트 안에 진단으로 있어 판정 승격만 하면 된다. **결과 보기 전에 '무엇을
      재실행할지' 를 고정**(사후 선택 방지). 그 전까지 35% 유지 — registry `winrate_threshold_decision_2026_09_15`
...
- validate_funding_cost.py: 펀딩비를 손익에 부과 — OKX(+합치 게이트 통과 시 Bybit) 정산 이력을
...
- validate_altseason_fast.py: 알트 시즌 **저지속성(차분·백분위) 판** — level 판에서 타깃·귀무·judge 를
  그대로 import 하고 지표 구성만 바꾼다(테스트가 `is` 로 동일성 고정). accel 이 MDE 를 2~4배 낮췄으나
  그 해상도에서 IC 가 전부 자기 MDE 아래 — **검정력이 아니라 신호 부재**. 기각 기록용.
  test_altseason_fast.py(60) / report_altseason.md 부록 B
- power_curve.py: 알트 시즌 시험의 **검정력 곡선** — 판정이 아니라 계산이다(test 가 verdict/judge/DEPLOY
  부재를 소스로 고정). 부분창 길이를 키우며 회전 검정 MDE 를 재고 log-log 로 MDE=c·n^(-p) 적합,
  **p 를 가정하지 않고 측정해서 외삽**한다. 데이터 구매 전 '사면 답이 나오는가' 에 답하는 용도.
  test_power_curve.py(33) / report_altseason.md 부록 A
- validate_altseason.py / upbit_feat 무관, data_long 전용: 알트 시즌 선행 지표 120일 지평 사전 등록 시험.
  **회전 검정 귀무**(지표만 >=180일 원형 이동 — 자기상관 보존·짝만 파괴)로 중첩 라벨의 가짜 유의를 상쇄하고,
  **MDE(귀무 IC 95백분위)를 판정에 넣어 검정력 부족을 기각과 구분**한다. 음성 대조 2(AR(1)·사인파)가
  '자기상관만으로 |IC| 0.18' 을 보여 준다 — 기각 기록용. test_altseason.py(49) / report_altseason.md
- validate_upbit_bsqueeze.py: B형(무거래 수축) 전용 — A형 풀 제외 + 수축 오름차순 + 라벨 지평 20봉.
  앞선 판이 내림차순만 써서 못 본 자리. test_upbit_bsqueeze.py(40)
- upbit_feat.py / validate_upbit_shoot.py: 업비트 KRW 일봉 특성(참조 study_upbit_shooting.features 와 비트 일치,
  test §1 이 고정) + 슈팅 선행 신호 사전 등록 시험(일별 횡단면 상위 10% lift, 날짜별 순열 귀무를 12 특성이 공유,
  음성 대조 3종, 월 창 분할). 판정 REJECTED — 기각 기록용. test_upbit_shoot.py(54) / report_upbit_shoot.md
...
- method_t.regmap_ready / outcome_d 1회성 경고: REGMAP 이 비면 레짐 전환 청산이 **조용히**
  사라지는 것(2026-09-21 결함)을 로그로 드러낸다. 동작 무변경
...
- elliott_count.json / elliott_watch.py: BTC 엘리엇 카운트 동결 + 일일 판정 (브리핑 전용, 매매 무관).
  라벨은 JSON 에만 있고 코드는 판정만 한다 — 재라벨은 revisions 에 사유와 함께 커밋으로 남긴다. test_elliott_watch.py(33)
