...
- **레짐 전환 청산이 꺼진 채 돌던 연구 2건 — 결함 발견·수정 (2026-09-21)**: registry
  ...
  **레짐 전환 청산**을 `REGMAP.get(date) not in (None, entry_reg)` 로 판정하는데, 모듈 기본값이
  ...
  · **재실행은 안 했다** — 관찰 기간 중이고 채택된 사이징 상수의 근거를 다시 여는 일이라 **사용자 결정**.
...
## 다음 할 일
- [ ] **② 펀딩비 시험 결과 판독** (funding_cost.yml) — MATERIAL 이면 배포 셀의 실효 엣지를 다시
      쓰고, IMMATERIAL 이어도 '커버 창이 bear 지배라 하한'이라는 단서를 함께 기록
- [ ] **③ 형성 중인 봉 시험 결과 판독** (forming_bar.yml) — MATERIAL 이면 기존 패턴에
      `detect_on_closed_bar` 를 켜는 것이 후속인데 **실거래 규칙 변경이라 사용자 결정 + 관찰 기간 이후**
- [ ] **⑤ 포트폴리오 변동성 타겟팅 결과 판독** (port_vol.yml) — J1~J6 전부 통과해야 후보,
      통과해도 사이징은 사용자 결정
- [ ] **sizing_vol / validate_portfolio 재실행 여부 결정 (사용자)** — 레짐 전환 청산이 꺼진 채
      돌았다(2026-09-21 발견). arm 비교는 상쇄되지만 절대 수준은 실거래 규칙이 아니었다.
      재실행은 채택된 RISK_FRAC/LEV_CAP/MAX_POS 의 근거를 다시 여는 일
