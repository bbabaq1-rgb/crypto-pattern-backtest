...
- **원장 결함 — 엔진 밖 전량 청산이 전부 '손절'로 기록된다 (2026-09-13 발견, 사용자 결정 대기)**:
  ...
  · **왜 중요한가**: 10/06 관찰 보고가 셀별 **손절 비율**을 집계한다(ih/marubozu/three_soldiers 정지·유지 근거).
    라벨이 틀리면 그 표가 틀린다.
  · 고치면 `reason` 문자열만 바뀌고 **주문·청산 동작은 안 바뀐다**. 관찰 기간이라 실거래 파일 수정은 사용자 결정 —
    (a) 지금 라벨만 수정 (b) 10/06 때 원장 사후 보정 (c) 무조치.
