...
- **월 +1,000% 목표 실현가능성 시험 — 불가 판정, 실거래 무변경 (2026-09-06, 사용자 지시 "수단 방법 가리지 말고
  월 1,000%")**: report_target_1000pct.md / study_target_1000pct.py(외부 데이터 불필요). 목표 = 월 로그수익 2.303,
  현행 실측(boot CAGR +63%) 0.041 의 **56배**, 역사상 최고 펀드(Medallion 연 66%)의 55배. 레포 실측 분포(승률 44%·
  손절 −1R·건당 +0.375R·월 30건)를 고정하고 위험 비율만 0.5→100% 스윕(상관·수수료·청산 0 = 목표에 유리한 가정):
  **켈리 최적 f*=10% 의 이론 상한이 월 1.9배**, 필요 건당 성장 0.0768 은 켈리 천장 0.0213 의 4배 — 사이징으로 못 넘는다.
  P(월 10배) 최대는 위험 30% 의 28% 인데 그 자리 중앙값 0.72x·킬스위치 33%(복권). 올인은 10% 확률로 10배 찍고 파산 100%.
  월 10배가 중앙값이 되려면 승률 44% 에서 승자평균 5R(+40%, 현 2.1R) 이상 — 106건 시험에 그런 셀 없음. 모델 자체가 현행
  1.5% 지점에서 실측보다 약 4배 낙관(월 1.15x vs 연 1.63x). 시장 조작·내부정보는 지시와 무관하게 안 한다.
  **RISK_FRAC/LEV_CAP/MAX_POS 불변.**
