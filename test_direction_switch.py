...
direction_switch 라우팅 오버라이드 검증 (네트워크 없음).
  ...
  - (bear, fvg) 는 FLAT — 2026-09-04 OFF → 2026-09-05 ON(베이스라인 수정 후 top20 PASSED) → 같은 날 저녁 다시 OFF
    (사용자: "2026년 한 해만 이익인 규칙은 적용 안 함"). regime_switch.json 이 short 라 해도 표는 FLAT 이어야 한다
...
# 2. 오버라이드 표 자체
check("override: (bear, fvg) 는 FLAT (2026-09-05 저녁 사용자 결정 — 2026 단일 해 이익)", ds.ROUTING_OVERRIDES.get(("bear", "fvg")) == "FLAT")
check("override: 그 외 항목 없음", set(ds.ROUTING_OVERRIDES) == {("bear", "fvg")})
...
try:
    ...
    check("저장소 direction_switch.json: bear/fvg FLAT (2026-09-05 저녁 OFF 복원)",
          real["routing"]["bear"]["fvg"] == "FLAT")
    check("저장소 direction_switch.json: overrides 에 bear/fvg FLAT", real.get("overrides", {}).get("bear/fvg") == "FLAT")
