...
# **동시 신호에서 '먼저 끝날 거래'를 고르면 안 된다** — 청산 시각을 정렬 키로 쓰면
# 빠른 청산(= 익절)을 미리 아는 셈이다. run 34315644631 에서 건당 −0.27% 규칙이
# 포트 +89,495배로 찍힌 원인. 같은 진입 시각이면 **입력 순서**로 잡아야 한다.
same_t = [(1.0, 9.0, -0.082),   # 먼저 온 느린 손실
          (1.0, 1.1, +0.008)]   # 같은 시각의 빠른 익절
pc = T.pot_curve(same_t, 1, 100.0)
check("동시 신호는 입력 순서로 잡는다 — 빠른 익절을 먼저 고르지 않는다",
      pc["taken"] == 1 and abs(pc["mult"] - (1 - 0.082)) < 1e-9, pc)
check("그 결과 포트는 손실을 실제로 떠안는다(낙관 편향 제거)", pc["mult"] < 1.0)
rev = T.pot_curve(list(reversed(same_t)), 1, 100.0)
check("입력 순서가 바뀌면 잡는 거래도 바뀐다(순서가 결과를 정한다는 성질 명시)",
      abs(rev["mult"] - 1.008) < 1e-9, rev)
check("정렬 키에 청산 시각이 없다",
      "recs[i][0], i" in inspect.getsource(T.pot_curve)
      and "sorted(recs)" not in inspect.getsource(T.pot_curve))
