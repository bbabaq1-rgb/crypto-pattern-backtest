...
#
# 하향 1.0% (2026-09-10, 사용자 결정 "1%/3배로 적용해줘" — 입금으로 equity $763 이 된 뒤):
#   낙폭 선호를 한 단계 낮춘 결정이다. 격자상 1.5%/lev3 -> 1%/lev3 의 뜻:
#     boot CAGR +68.7% -> +49.1% / boot MDD중앙 -60.8% -> -45.4% / p10 -80.5% -> -65.1% /
#     P(ruin) 2.3% -> 0.3% / Calmar 1.13 -> 1.06. **수익도 낙폭도 약 3분의 2** — Calmar 가
#   거의 안 움직이는 것이 '더 좋아진 게 아니라 볼륨을 낮춘 것'이라는 증거다(격자 15셀 0.99~1.14).
#   여전히 사전 기준(MDD중앙 >= -35%)은 미충족 — 통과하는 건 0.5% 뿐이고 그건 이 계좌에서
#   최소주문 문턱이 $1,067 이라 주문 자체가 안 나간다.
#   · **총 명목가 상한(2.5x)이 풀린다** — 건당 명목가가 equity 의 18.75% -> 12.5% 가 되어
#     상한에 걸리는 지점이 13.3개 -> **20개**. 종전에는 MAX_POS 16 을 구조적으로 못 채웠다
#     (2026-09-10 포트폴리오 프레임에서 holdout 병목이 슬롯 440 vs 증거금 2,750 으로 나온 자리).
#   · **대가: 최소주문 문턱이 risk 에 반비례해 올라간다** — 최고변동(배율 0.45) 신호까지
#     통과하는 equity 가 $355 -> **$533**. 현 계좌 $763 에서는 전부 통과하나 여유가 $230 이다.
#   · lev 는 3 유지(아래 참조). 1%/lev2 였다면 증거금·문턱이 1.5%/lev3 과 **완전히 같아져**
#     (둘 다 6.25% / $355) 명목가만 3분의 2로 줄었을 것이다 — lev 3 은 거기에 더해 포지션당
#     증거금을 6.25% -> 4.17% 로 낮춰 16슬롯을 실제로 채울 수 있게 한다.
#
# **2026-09-18 RISK_FRAC 1.0% -> 1.5% 재상향 (사용자 결정 "리스크 1.5%로 올리고 맥스포스 13으로
# 같이 내려줘")** — 9/10 하향을 되돌리되 **MAX_POS 를 16 -> 13 으로 함께 내린다.** 둘은 한 쌍이다:
#   risk 1.5% 에서 건당 명목가는 equity 의 18.75% 이므로 총명목 상한(2.5배)이 **13.3개**에서 걸린다.
#   MAX_POS 16 을 그대로 두면 14번째부터 상한 스킵으로 바뀔 뿐이라 슬롯 숫자가 허상이 된다.
#   포지션당 증거금도 4.17% -> 6.25% 라 13슬롯이 equity 의 81%(16슬롯이면 100%).
# 뜻: boot CAGR +49.1% -> **+68.7%** / MDD중앙 -45.4% -> **-60.8%** / p10 -65.1% -> **-80.5%** /
#   P(ruin) 0.3% -> 2.3% / Calmar 1.06 -> 1.13. **5번 중 1번 -80% 낙폭**을 받아들인 선택이고,
#   사전 기준(MDD중앙 >= -35%)은 여전히 미충족이다(통과 셀은 0.5% 뿐, 그건 문턱 $1,065 라 현
#   계좌에서 주문이 안 나간다). 근거는 격자 15셀 Calmar 0.99~1.14 — 최적 위험은 없고 낙폭 선호가 정한다.
# 부수: 최소주문 문턱이 risk 에 반비례해 **$533 -> $355** 로 내려간다(배율 1.0 기준 $240 -> $160).
#   equity $606 기준 여유가 $73 -> $251 로 늘어 고변동 신호 스킵 위험이 줄어든다.
# 되돌리기: 이 줄을 0.010 으로, paper_executor.MAX_LIVE_POS / sizing_study.MAX_POS 를 16 으로.
RISK_FRAC               = 0.015  # 건당 감수 위험 = equity 의 1.5% (사용자 결정 2026-09-18, 문턱 $160)
#
# LEV_CAP 3 (2026-09-04, 사용자 결정 — RISK_FRAC 1.5% 상향 직후 "lev 3 으로 올려줘"):
#   2026-09-02 의 '레버리지 상향은 이득 없이 MDD 만 악화'는 **risk 1% 에서의 결론**이었다.
#   거기서는 증거금이 남아돌아(격자에서 증거금 스킵 0건) 레버리지가 살 수 있는 게 없었다.
#   risk 1.5% 에서는 사정이 다르다 — equity $400 에서 포지션당 증거금 $37.5(lev 2)라 MAX_POS 12
#   를 채우려면 $450 이 필요해 10개쯤에서 free 가 마른다. 격자(라우팅 991건):
#     risk 1.5% / lev2: 증거금스킵 109건, boot Calmar 1.03, MDD중앙 -59.7%, P(ruin) 2.3%
#     risk 1.5% / lev3: 증거금스킵  15건, boot Calmar 1.13, MDD중앙 -60.8%, P(ruin) 2.3%
#   즉 **레버리지가 실제로 진입을 사 온다**(94건). MDD 중앙은 -1.1%p 더 깊어지지만 CAGR 이
#   +63.0% -> +68.7% 로 더 올라 Calmar 가 개선된다.
#   안전: 8% 손절에서 lev 3 의 청산 거리는 1/3 - MMR = 32.3% 로 손절폭의 4.0배 — LIQ_SAFETY 2.0
#   기준을 여유 있게 만족한다(lev 2 는 49%/6.1배). liq_safe_leverage 상한 5 보다도 낮다.
#   **대가: 최소주문 문턱이 레버리지에 비례해 올라간다.** 최고변동(배율 0.45) 신호까지 통과하는
#   equity 가 lev2 $237 -> lev3 **$355**. 현 계좌($400)에서는 여전히 전부 통과하지만,
#   **equity 가 $355 밑으로 내려가면 고변동 신호부터 주문이 안 나가기 시작한다.**
#   **2026-09-10 risk 1.0% 하향 시에도 3 을 유지한다** — 위 '무의미했던' 서술은 risk 1% /
#   MAX_POS 12 / equity $400 시절의 것이고, 지금은 MAX_POS 16 이라 lev 2 면 16슬롯 증거금이
#   equity 의 **100%** 가 되어 다시 못 채운다(lev 3 이면 67%). 대가로 문턱이 $355 -> $533 이나
#   현 계좌 $763 이 그 위다. lev 2 로 내리면 문턱은 $355 로 낮아지는 대신 증거금 제약이 돌아온다.
LEV_CAP                 = 3      # 레버리지 상한 (사용자 결정 2026-09-04, 2026-09-10 risk 1% 에서도 유지)
